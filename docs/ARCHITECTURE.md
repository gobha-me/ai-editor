...
> Last sync: **2.68.0** (2026-05-19, ninth RE-EVAL slot — `RE-EVAL following 2.64.0`, on-cadence (first slot since methodology adoption to fire before the canonical 5-minor overdue mark; only 4 minors past anchor at run-time); doc-only — no version bump, accumulates in [Unreleased]; 2-minor catch-up from the 2.66.0 sync covering: **2.67.0 `Profiles` namespace public-surface shape-pinning test** (ICD #8 finding #2; new `tests/test-profile-registry-shape.mjs` with 9 subtests against six shape contracts — `registry.js` 6-name surface + `resolve.js` 13-name surface + `Profiles` 5-method shape + `BY_NAME` 10-name membership + `Profiles.list()` insertion-order pin + `PLUGIN_TOOL_NAMES` freeze; capabilities-precedent idiom; zero production-file edits; closes ICD §Open invariant #3), and **2.68.0 align 3 short-circuit resolvers with `resolveProfile`** (ICD #8 finding #1; `resolveScriptAutomationConfig` / `resolvePreviewConfig` / `resolvePluginConfig` rewritten to mirror the other six `resolve*Config` helpers' `Profiles.has` → `Profiles.get` → `resolveProfile` chain; pre-emptive fix — no production profile inherits via `base: 'coder.v1'` today, but the 2.0.x advanced-view picker is the natural pull; `CODER_V1`/`CHAT_V1` imports retired from `js/profiles/resolve.js`; 18 subtests across three new sibling test files covering value-cases + chat.v1 defaults + synthetic-profile inheritance-walk pin over 6 chat.v1 inheritors + every-registered-profile clean walks + defensive fallbacks; closes `RE-EVAL following 2.61.0`'s code-aware cohort). Two precedent shape-pinning idioms continue as settled patterns: **capabilities-precedent** (direct `Object.keys` deepEqual; 2.50.0 + 2.63.0 + 2.67.0) for public-export pins, **turn-stats-precedent** (validator-at-a-seam; 2.62.0) for EventBus-coupled payload contracts. Per-subsystem detail lives in [`docs/DESIGN-*.md`](.) and [`docs/ICD-*.md`](.) — ninth ICD authored this slot at [`docs/ICD-editor-instance.md`](ICD-editor-instance.md) (editor instance + tab-manager — the last named ICD-backfill target; `editorInstance` singleton + CM6 loader/namespace + three opt-in decoration Compartments (blame data-driven / invisible-Unicode per-file / ghost-text settings-driven with four-state machine) + line/range mutation surface + typed-tab dispatch; 9 files / ~2983 LOC under one ICD; 5 axes: Loading / Lifecycle / Editing / Decoration / Tabbing; keymap-compartment-before-basicSetup ordering pinned as the most load-bearing invariant; seven open invariants noted at ICD-author time); the ICD-backfill program named target list is now exhausted — every target #1–#9 has shipped its ICD. This doc tracks structural shape only.
...
CodeMirror 6 setup + instance + three opt-in Compartment decorations. **`instance.js`** (953 LOC) owns the `editorInstance` singleton + `createEditor(container, content, filename)` async boot + 22 named exports (content / cursor / line-range / Compartment toggles). The keymap-compartment-before-basicSetup ordering invariant ([`instance.js:74–98`](../js/editor/instance.js)) is load-bearing: reordering would silently silence Vim mode + ghost-text Tab/Esc bindings because CM6 evaluates extensions in order and earlier registrations win. **`setup.js`** (436 LOC) carries the 79-field `CM` namespace + `loadCodeMirror()` vendor-bundle-preferred-with-CDN-fallback loader + `getLanguageExtension(filename)` 17-language dispatch; CDN-path assignments use per-extension `?.` for graceful degradation. **`ghost-text.js`** (551 LOC) hosts the `idle → requesting → showing → idle` state machine driven by four `StateEffect`s + a module-local `_inFlight` flag + `_activeAbort` `AbortController`; network promise lives outside CM transactions by design; Tab/Esc keymap with an indent carve-out + `?ghostText=off` URL kill-switch. **`blame-gutter.js`** (251 LOC) installs an empty Compartment + populates it via `setBlameData(view, ranges)` — the gutter column disappears when data is cleared. **`invisible-unicode-decoration.js`** (215 LOC) inline-widgets zero-width / bidi-override / glassworm tag-block chars; click-to-delete + `Mod-Shift-U` strip-in-selection. **`file-utils.js`** (217 LOC) is the pure-function text/binary detector (no CM dependency); **`diff.js`** (51 LOC) is the post-edit-confirmation diff formatter. The barrel **`editor.js`** (48 LOC at the project root) selectively re-exports 19 symbols; deep imports are reserved for `plugin-editor.js` (CM namespace + loader), `secondary-pane.js` (blame + `editorInstance`), `pr-review/PrReviewDock.js` (`applyEdit`).
