...
> Last sync: **2.66.0** (2026-05-19, eighth RE-EVAL slot — `RE-EVAL following 2.61.0`, overdue by 5 minors past anchor; doc-only — no version bump, accumulates in [Unreleased]; 3-minor catch-up from the 2.63.0 sync covering: **2.64.0 `destroy()` plugin hook invocation** (ICD #7 finding #1; strict symmetric mirror of the first-enable init antibody in `Plugins.setEnabled` at `js/core.js:1227-1234`; `manifest.destroy?.(instance, config)` runs on first-time disable when an instance was captured, catches/logs errors, clears instance for fresh re-init; 4 subtests in `tests/test-plugin-lifecycle.mjs`), **2.65.0 invisible-Unicode scan findings persisted to `InstalledPlugin` record** (ICD #7 finding #3; opt-in `invisibleUnicodeFindings?: Array<{codepoint, name, line, col}>` field on `InstalledPlugin`; new `_buildInstalledRecord` helper in `js/plugin-loader.js` attaches the four-key audit subset; Settings → Plugins external-row gains `⚠ N flagged at install` badge with per-finding tooltip; back-compat-automatic — field absent on pre-2.65.0 records; 5 subtests at `tests/test-plugin-loader-invisible-unicode-persist.mjs`), and **2.66.0 retire auto-profile-switch on plugin-editor tab** (ICD #7 finding #2 resolution C; the `tab:switched` listener + `_savedProfile` + `_syncProfileUI` deleted entirely from `js/plugin-editor.js`; the 2.58.0 `plugin.enabled` capability overlay is now the sole non-destructive admission path for the 5-tool plugin-dev cohort; in-tab opt-in banner via `_readPluginOverlayEnabled` / `_enablePluginOverlay` module-locals reusing `resolvePluginConfig` from `js/profiles/resolve.js`; all three pre-2.66.0 failure modes — per-chat binding divergence + capability-overlay redundancy + restore-target staleness — eliminated; 4 subtests at `tests/test-plugin-editor-auto-switch-retired.mjs`). Two precedent shape-pinning idioms continue as settled patterns: **capabilities-precedent** (direct `Object.keys` deepEqual; 2.50.0 + 2.63.0) for public-export pins, **turn-stats-precedent** (validator-at-a-seam; 2.62.0) for EventBus-coupled payload contracts. Per-subsystem detail lives in [`docs/DESIGN-*.md`](.) and [`docs/ICD-*.md`](.) — eighth ICD authored this slot at [`docs/ICD-profiles-registry.md`](ICD-profiles-registry.md) (post-2.54.0 profile-side admission contract + inheritance composition + resolver pattern across `js/profiles/` 18 files / ~3637 LOC, 5 axes: Declaration / Admission / Inheritance / Resolution / Diagnostics — the post-inversion pair of the superseded ICD #3 tool-side admission boundary); this doc tracks structural shape only.
