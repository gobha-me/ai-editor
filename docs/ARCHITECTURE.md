...
> Delivery/product authority reset 2026-09-01: the long version-stamped sync
> narrative below is preserved as historical evidence, not a cadence contract.
> Last sync: **2.86.0** (2026-05-22, eleventh RE-EVAL slot — `RE-EVAL following 2.70.0`, overdue by 5 missed anchors (would-be slots at 2.70, 2.73, 2.76, 2.79, 2.82, 2.85 all skipped; we're at 2.86.0); single-slot absorption per slot #10 precedent; doc-only — no version bump, accumulates in [Unreleased]; **11-minor catch-up** from the 2.75.0 sync — the largest single-slot delta to date, exceeding slot #6 / slot #10's prior 8-minor record by 3. Covers: **2.76.0** plan-mode tool-gate moved to dispatch boundary (gitea#480) — new `js/intelligence/tools/side-effects.js` extracted from `catalog.js` to break the registry↔catalog cycle, `ToolRegistry.checkPlanModeAccess(name)` authoritative gate at the top of `executeWithProfile` before the role check, `PLAN_MODE_SESSION_WRITE_ALLOWLIST` for session-local writes (`scratchpad_write`, `todo_write`, 5 preview action tools), 16 missing tool classifications added, new `tests/test-plan-mode-source-scan.mjs` catalog-parity lint with 3 subtests + 8 dispatch-side subtests in `tests/test-plan-mode.mjs`; **2.77.0** approved plan durable across context window (gitea#478) — `<approved-plan-current>` passive-push block prepended in `buildSystemPrompt` before the `PLAN MODE ACTIVE` block whenever `getApprovedPlan()` is non-null, anti-resubmit nudge text + ISO `approvedAt` attribute, slot still cleared on conversation switch via existing `clearApprovedPlan()`; **2.78.0** T1 tool-authored failure shape conformance (sized parallel-work slot) — every tool-side rejection in the 11-handler `T1_CONFORMANT_TOOLS` cohort carries `code:` from a `VALID_CODES` closed set alongside the existing human-readable `error:` narration; `code:` field choice (over `failure_code:` alternative) aligns tool-throw + tool-return failures on one envelope shape, pinned in `docs/DESIGN-tools.md` §"Tool-authored failure shape contract"; recovery-sufficient payload additions (`edit_file` stale-line returns gain `suggested_start_line`/`suggested_end_line`/`current_content_window` named fields; `read_lines` invalid-line cases gain `actual_file_line_count`); `run_plugin`'s hybrid `{success: false}` envelope normalized away per DESIGN-agent-loop's Authorship Rule; new `tests/test-tool-failure-shapes.mjs` source-scan lint walks `js/tools/*.js` and asserts every `register('NAME', HANDLER, ...)` resolved handler in `T1_CONFORMANT_TOOLS` returns the `{error, code}` shape with `code` ∈ `VALID_CODES`. **Intelligence-layer design-doc pack landed in the same release**: 5 replaced design docs (`docs/DESIGN-intelligence.md`, `docs/DESIGN-memory.md`, `docs/DESIGN-profiles.md`, `docs/DESIGN-tools.md`, `docs/DESIGN-compression.md`) + 3 new docs (`docs/DESIGN-INDEX.md` as the 8-doc set index, `docs/DESIGN-agent-loop.md` as the consumer surface below the four admission subsystems, `docs/DESIGN-persona.md` as identity composition above Profile — N/A for ai-editor's coder-only product per ROADMAP §"What's out of scope") + 1 archived (`docs/DESIGN-CHANGES-2026-05-21.md` preserved as historical evidence per the pack's own framing); **2.79.0** `read_file` refreshes `EditTracker.recordRead` (gitea#485) — closes a missing-call gap where `read_file` was the only read tool that didn't reset the staleness clock; siblings `read_lines` / `read_current_file` / `open_file` already did; root cause was an omission at sibling-add time, not the cache-invalidation-on-mutation pattern; new `tests/test-edit-tracker-read-tool-contract.mjs` source-scan lint pins all four read tools at once (4 subtests); **2.80.0** `commit_files` surfaces `write_file`-auto-committed files via `created` array (gitea#486) — new `js/tools/_session-auto-commits.js` exports a session-scoped `Set<string>` with three pure helpers (`recordAutoCommit`, `getAutoCommittedSinceLastReport` drains, `clearAutoCommitted` full clear); `write_file`'s new-file branch records the path after `Git.createFile` succeeds + response also gains `committed: true` + `commit_message`; `commit_files` drains the tracker into `response.created` on success + the "no dirty files" early return; lifecycle paired with `clearApprovedPlan` at three `js/chat/conversations.js` sites (conversation switch, new chat, delete); 4 subtests; **2.81.0** `search_in_files` fails loud when truncated (gitea#487) — module-scope `MAX_FILES = 500` (was hardcoded 50 inside the loop); truncated-with-zero-matches branch returns `{error, code: 'search_truncated', query, files_searched, total_files_in_scope}` instead of the success envelope; truncated-with-results keeps the prior success+`files_truncated`+`hint` shape; 4 subtests; **2.82.0** anti-loop refusal no longer suggests functionally-unrelated tools (gitea#488) — `suggestions` field removed entirely from `buildRefusalPayload` + 3 picker helpers (`_hash32`, `_pickDeterministic`, `SUGGEST_COUNT`) deleted; new "change at least one argument before retrying" off-ramp prose; `Catalog` import dropped from `tool-loop-core.js`; specs in `tests/test-tool-loop-suggestions.mjs` inverted; **2.83.0** agent-loop contract centralization (post-`docs/DESIGN-agent-loop.md`) — new JSDoc-only `js/chat/agent-loop-contracts.js` module + Authorship Rule classification table (10 rows verbatim from the design doc) + `EnvelopeShape` union (`SuccessEnvelope` / `RefusedEnvelope` / `CachedEnvelope` / `PartialEnvelope` — flag-presence discrimination, no `kind` field added to runtime envelopes) + `LoopState` typedef (`noProgressStreak`, `roundCount`, `duplicateStreak`, `toolCallCache`, `toolActionLog`, `queuedInputFIFO`, `userPausePromise`) + `CacheResult` + `DupStreakPolicy`; 4 consumers (`tool-loop-core.js`, `cache-invalidation.js`, `cache-policy.js`, `refusal-hints.js`) cite via `@see` module-header pointer + JSDoc `@type`/`@returns` annotations on envelope-construction sites; `PartialEnvelope` documented even though no current emitter — the whole point of centralizing the un-emitted shape; new `tests/test-agent-loop-contracts-citation.mjs` source-scan lint (7 subtests) reads each consumer as text and asserts the literal `agent-loop-contracts` substring is present; **2.84.0** `create_pull_request` refuses with uncommitted changes (gitea#493) — dirty-tab precondition at the top of the handler after the no-project guard + before the head/base check; filters `State.openTabs` for `dirty` non-issue tabs (issue tabs carry no working-tree state); returns `{error, code: 'uncommitted_changes', dirty_paths}` — `dirty_paths` serves the model's recovery step directly without re-derivation; `uncommitted_changes` added to `VALID_CODES` but `create_pull_request` not graduated to `T1_CONFORMANT_TOOLS` (precedent: 2.80.0/2.81.0); 4 subtests; **2.85.0** Rule 3/4 coverage measurement probe + log/debug pane button — new pure-Node-importable `js/chat/rule3-coverage-probe.js` (sibling of `metadata-probe.js`, ~310 LOC) exports `probeRule3Coverage(history, opts) → Rule3Report` + `formatRule3ReportMarkdown` + `formatRule3ReportJSON` + `summarizeRule3Coverage` + frozen `CURRENT_DISPATCH_PATHS`; three-bucket classifier (a populated+resolves; b present-but-unresolvable; c absent — gate-excluded per cross-revision-tolerance); 9-path first-match-wins dispatch inferer (`refused-envelope`, `partial-envelope`, `cross-request-cache-hit`, `same-request-cache-hit`, `tier0-sandbox`, `mcp-bridged`, `sub-agent`, `plan-mode-post-approval`, `direct`); historical-path flagging via `CURRENT_DISPATCH_PATHS` frozen set with maintainer doc-comment ("when retiring a path emitter, remove its name") — the lightest cross-revision-tolerant signal compatible with the no-build constraint; gate computation `pct = passing / eligible * 100` with `threshold = 95` default; markdown formatter has stable header for paste-into-issue flows + snapshot pinning; JSON formatter sorts keys for byte-stable output; button wired into Logs tab of `js/debug-slideout.js` via existing `setTimeout(...,0)` chip-handler idiom (dynamic-imports the probe — lazy load); `tests/test-rule3-coverage-probe.mjs` 37 subtests pin gate computation + dispatch-path inference + historical-path detection + formatter stability; **2.86.0** same-tool-name streak guard (gitea#496) — `SAME_TOOL_REFUSE_THRESHOLD = 5` sibling counter to the args-exact `DUP_REFUSE_THRESHOLD = 3`, OR-gated at the refusal check; `LoopState` typedef gains `sameToolStreak` + `lastInvokedToolName` fields; `RefusedEnvelope` typedef updated to name both trigger paths (args-exact and same-name); `DupStreakPolicy` typedef gains `thresholdSameName` field; `buildRefusalPayload` gains `opts.variedArgs` flag (the loop passes `true` when the same-name guard fires alone); envelope prose swaps `"with identical args"` → `"with varying args"` + `"change at least one argument"` → `"call a different tool entirely"` + `"Do not call X again with these arguments"` → `"Do not call X again — varying args has not produced progress"`; legacy `variedArgs: false` path byte-identical for back-compat; gitea#496 origin = qwen-3-6-plus on 2.84.0 defeated the args-exact guard by varying one byte per call (`{path:'foo'}` → `{path:'foo', limit:100}` → `{path:'foo', limit:200}` → …); 8 subtests in new `tests/test-tool-loop-anti-loop.mjs` pin OR-gate semantics + threshold boundary + reset on different-tool-name + accurate prose on both trigger paths. The three precedent shape-pinning idioms continue as settled patterns: **capabilities-precedent** (direct `Object.keys` deepEqual; 2.50.0 + 2.63.0 + 2.67.0 + 2.72.0) for public-export pins; **turn-stats-precedent** (validator-at-a-seam; 2.62.0) for EventBus-coupled payload contracts; **source-scan-precedent** (read file → `stripComments` → regex-match in extracted function body; 2.44.0 + 2.66.0 + 2.71.0 + 2.72.0 + 2.75.0 + 2.79.0 + 2.80.0 + 2.81.0 + 2.83.0 + 2.84.0) for code-shape invariants that don't reduce to export-surface deepEqual — six new uses across 2.79.0–2.84.0 cement this as the most-applied idiom in the program. A fourth pattern is now solidifying — **closed-code-set lint** (T1 conformance at 2.78.0): a `VALID_CODES` constant + source-scan over `js/tools/*.js` `register('NAME', HANDLER, ...)` sites asserts every structured-return-error site in a `T1_CONFORMANT_TOOLS` handler carries a `code:` field from the closed set; the pattern composes with source-scan-precedent (handler resolution shares the brace-walk extractor) but the gate is set-membership rather than per-site shape. Per-subsystem detail lives in [`docs/DESIGN-*.md`](.) and [`docs/ICD-*.md`](.). **No new ICD authored this slot — the ICD-backfill program named target list (#1–#9) remains exhausted as of 2.68.0**; this slot ran the paper-half (ARCH catch-up + ROADMAP drift audit) + code-aware halves (deep sweep ICDs #1/#3/#8; confirmation pass ICDs #2/#4/#5/#6/#7/#9); ICD #1 absorbed three doc edits (agent-loop-contracts cross-reference + same-tool-name streak guard fields + Rule 3/4 probe reference) + ICD #3 absorbed one doc edit (T1 failure-shape contract as second post-supersedence note after 2.71.0 `cache:` field); see ROADMAP §"Just ran (RE-EVAL following 2.70.0)". Original slot #10 leader retained: **2.75.0** (2026-05-20, tenth RE-EVAL slot — `RE-EVAL following 2.67.0`, overdue by 5 minors past anchor (we're at 2.75.0; slot was due at 2.70.0); same overdue-by-5 shape as slots #6/#7/#8 after slot #9 briefly fired on-cadence; doc-only — no version bump, accumulates in [Unreleased]; **8-minor catch-up** from the 2.68.0 sync — the largest single-slot delta to date (slot #6 had the only prior 8-minor lift; slots #7/#8/#9 were 3/3/2 minors). Covers: **2.69.0** Gitea `compareRefs` schema-correction `files: []` shape pin + `getChangedFilesBetween` Gitea override (14 subtests in [`test-gitea-compare.mjs`](../tests/test-gitea-compare.mjs); silent-empty retrieval delta-index fix); **2.70.0** `createBranch` idempotent-on-existing-ref contract (Gitea 500 + `PushRejected`/`reference already exists` + branch-name-in-body guard, GitHub 422 + `Reference already exists`; `git:branchCreated` emits only on the genuine-creation path; 11 subtests in [`test-create-branch-idempotency.mjs`](../tests/test-create-branch-idempotency.mjs); GitLab cohort-closure deferred to 2.74.0); **2.71.0** structural fix — `[ToolDefinition.cache](../js/tools/registry.js)` field lifted onto registration site (`'by-args'` default / `'never'` bypass), retiring the cache-classification whack-a-mole pattern across four prior recurrence sites (gitea#301 / github#39 / 2.10.0 Tier 3a / gitea#472); 20 tools migrated to `cache: 'never'` at their `[js/tools/*.js](../js/tools/)` registration blocks; new [`test-tool-cache-classifications.mjs`](../tests/test-tool-cache-classifications.mjs) lint guard scans for stale-prone name shapes (`list_*` / `find_*` / `get_*` / `*_status` / `*_logs`) and rejects unclassified additions; the source-scan idiom mirrors [`test-chat-tool-name-literals.mjs`](../tests/test-chat-tool-name-literals.mjs); **2.72.0** ICD #9 cohort findings #2 + #4 — compartment-ordering invariant source-scan pin ([`test-editor-compartment-ordering.mjs`](../tests/test-editor-compartment-ordering.mjs); 3 subtests asserting `keymapCompartment.of` precedes `ghostTextCompartment.of` precedes `basicSetup`-push to keep Vim mode + ghost-text Tab/Esc bindings live) + `refreshGhostText` barrel export activation (silent live bug — [`persistence.js:92`](../js/settings/persistence.js)'s `m.refreshGhostText && m.refreshGhostText()` `&&`-short-circuit masked the missing export so ghost-text settings persisted but never reconfigured the live CM6 compartment; [`test-editor-barrel-shape.mjs`](../tests/test-editor-barrel-shape.mjs) capabilities-precedent shape-pin, 4 subtests); **2.73.0** PR-review draft-vs-conflict disambiguation (gitea#466) — `pr.draft === true` passthrough on both [`gitea.js`](../js/git-providers/gitea.js) + [`github.js`](../js/git-providers/github.js) `getPullRequest()`; new `pr-dock__notice` draft element + merge-controls disable in [`PrMergeControls.js`](../js/pr-review/PrMergeControls.js); `[base.js](../js/git-providers/base.js)` `PullRequestData.draft` typedef row pins the disambiguation contract (11 subtests in [`test-pr-review-draft-vs-conflict.mjs`](../tests/test-pr-review-draft-vs-conflict.mjs)); **2.74.0** GitLab parity cohort closure — `createBranch` idempotency (400 + canonical `Branch already exists`) + `getPullRequest` draft passthrough normalizing modern `draft` + pre-deprecation `work_in_progress` MR fields (+8 subtests across the two suites); **2.75.0** ICD #9 finding #1 closure — `editorInstance` destroy-then-replace contract source-scan ([`test-editor-lifecycle.mjs`](../tests/test-editor-lifecycle.mjs); 4 subtests pinning `editorInstance.destroy()` precedes rebind + Compartment-reassignment-once + module-scope `let` forward-evolution guard against silent promotion to `const` or move-to-function-local). Two precedent shape-pinning idioms continue as settled patterns: **capabilities-precedent** (direct `Object.keys` deepEqual; 2.50.0 + 2.63.0 + 2.67.0 + 2.72.0) for public-export pins, **turn-stats-precedent** (validator-at-a-seam; 2.62.0) for EventBus-coupled payload contracts. A third pattern is now solidifying — **source-scan precedent** (read file → `stripComments` → regex-match in extracted function body; 2.44.0 chat-tool-name-literals + 2.66.0 plugin-editor-auto-switch-retired + 2.71.0 cache-classifications + 2.72.0 compartment-ordering + 2.75.0 editor-lifecycle) for code-shape invariants that don't reduce to an export-surface deepEqual. Per-subsystem detail lives in [`docs/DESIGN-*.md`](.) and [`docs/ICD-*.md`](.). **No new ICD authored this slot — the ICD-backfill program named target list (#1–#9) is exhausted as of 2.68.0**; this slot ran the paper-half (ARCH catch-up + ROADMAP drift audit) and code-aware halves (sweep across all 9 established ICDs) only, surfacing N findings (see ROADMAP §"Just ran"). This doc tracks structural shape only.

> **Contract status.** Layer boundaries and interface contracts remain
> authoritative where they match code. Version-stamped cadence and commitment
> bands are historical evidence; current product promotion follows ROADMAP
> outcomes and observed failures.
...
- **No application build step.** Browser modules remain native ESM. The
  container's vendor bundle alone uses a locked npm graph and esbuild.
...
### `chat/agent-loop-contracts.js` (2.83.0, JSDoc-only)

Source-side citation point for the agent-loop consumer surface that sits between the four admission subsystems (retrieval / memory / compression / tools) and the LLM. The previously-unnamed surface was named in [`docs/DESIGN-agent-loop.md`](DESIGN-agent-loop.md) on 2026-05-21 — envelope-authorship rule, cache coordination (same-request + cross-request), dup-streak guard, no-progress guard, queued-input FIFO, user-pause Promise slot, stateful-read bypass, sub-agent inheritance. All contracts are correctly implemented today, but were scattered across four files in `js/chat/` with no architectural home; future cache-deadlock / dup-loop / envelope-authorship disputes had to do archaeology against implementation. The new module is JSDoc-only — no runtime exports, no behavior change — carrying the Authorship Rule classification table verbatim from the design doc (10 rows) + four typedefs: `EnvelopeShape` union over `SuccessEnvelope` / `RefusedEnvelope` / `CachedEnvelope` / `PartialEnvelope` (flag-presence discrimination via `_refused: true` / `_cached: true` / `_partial: true`; no `kind` discriminator field added to runtime envelopes); `LoopState` (`noProgressStreak`, `roundCount`, `duplicateStreak`, `sameToolStreak` 2.86.0, `lastInvokedToolName` 2.86.0, `toolCallCache` same-request LRU, `toolActionLog` cross-request, `queuedInputFIFO`, `userPausePromise`); `CacheResult` (the two-step lookup pattern in source today — same-request `Map.get(cacheKey)` then cross-request `findMatchingCrossRequestEntry`); `DupStreakPolicy` (`threshold`, `thresholdSameName` 2.86.0, `callKeyHash`, `intervening_progress_resets`). `PartialEnvelope` is documented even though **no current emitter exists** — naming the un-emitted shape gives future soft-budget work a named contract to land against without synthesizing a fake emitter today; this is the whole point of centralizing. Four consumers cite the module via `@see ./agent-loop-contracts.js` in their module header + JSDoc `@type`/`@returns` annotations: [`tool-loop-core.js`](../js/chat/tool-loop-core.js) (loop body; authors `EnvelopeShape`; owns `LoopState`), [`cache-invalidation.js`](../js/chat/cache-invalidation.js) (cache coordination; builds `CachedEnvelope` via `buildCrossRequestCacheResult`), [`cache-policy.js`](../js/chat/cache-policy.js) (`isStatefulRead` / `getStatefulReadToolsLive` per design §"Cache-Key Composition + Stateful Reads"), [`refusal-hints.js`](../js/chat/refusal-hints.js) (`buildRefusalPayload` authors `RefusedEnvelope`). Constants `NO_PROGRESS_LIMIT`, `HARD_CAP`, `DUP_REFUSE_THRESHOLD`, `SAME_TOOL_REFUSE_THRESHOLD` live at the top of `tool-loop-core.js` — NOT re-declared in the contracts module to keep it pure documentation and avoid import-shuffle cycle risk (the [`cache-policy.js`](../js/chat/cache-policy.js) module already flags load-time cycles around `tool-classifications.js`). New `tests/test-agent-loop-contracts-citation.mjs` source-scan lint reads each consumer as text and asserts the literal `agent-loop-contracts` substring is present — centralization without a test rots; the lint locks the citation in place across the four-consumer set (7 subtests: 4 consumer citation lints + 3 contracts-module self-checks).

**Same-tool-name streak guard (2.86.0; gitea#496).** `SAME_TOOL_REFUSE_THRESHOLD = 5` sibling counter to the args-exact `DUP_REFUSE_THRESHOLD = 3` (original 1.7.1 guard keyed by `(toolName, canonicalArgsKey(args))`). The args-exact guard was defeated in the field by qwen-3-6-plus on 2.84.0 — the model varied one byte of args per call (`{path:'foo'}` → `{path:'foo', limit:100}` → `{path:'foo', limit:200}` → …) so the exact cacheKey never repeated, `isDup` stayed false, the streak counter reset every call. The new counter is args-independent; OR-gate at the refusal check — refuse if **either** the args-exact streak hits 3 OR the same-name streak hits 5. Counter lifetime is request-scoped (declared inside `runToolLoop` alongside `duplicateStreak`); increments when `toolName === lastInvokedToolName`, resets to 1 when a different tool name is invoked. Legitimate exploration (5 different files in a row) trips the guard, but the off-ramp is simply "interleave any other tool call" — same shape as the existing exact-args guard. When the same-name guard fires alone (args genuinely varied), the loop passes `opts.variedArgs = true` to `buildRefusalPayload`; envelope prose swaps `"with identical args"` → `"with varying args"` + `"change at least one argument"` → `"call a different tool entirely"` + `"Do not call X again with these arguments"` → `"Do not call X again — varying args has not produced progress"`. Legacy `variedArgs: false` path byte-identical for back-compat; existing `tests/test-tool-loop-suggestions.mjs` passes unmodified. 8 subtests in new `tests/test-tool-loop-anti-loop.mjs` pin OR-gate semantics + threshold boundary + reset on different-tool-name + accurate prose on both trigger paths.

### `chat/rule3-coverage-probe.js` (2.85.0)

Pure-Node-importable measurement probe (sibling of `metadata-probe.js`, ~310 LOC; no `State` / `EventBus` / DOM imports). Exports `probeRule3Coverage(history, opts) → Rule3Report` + `formatRule3ReportMarkdown` + `formatRule3ReportJSON` + `summarizeRule3Coverage` + frozen `CURRENT_DISPATCH_PATHS`. Built as the gating prerequisite for clearing the Compression Rule 3 (Consumption) + Rule 4 (Resolution) deferrals — the ROADMAP gate is *"≥95% `tool_result_for` coverage on production sessions across ≥10 coder.v1 sessions"*; until 2.85.0 there was no mechanism to measure it. The report this probe produces IS the evidence Rule 3 ships against. Three-bucket classifier per turn (`role: 'tool'`): bucket a = `tool_result_for` populated and resolves to a `tool_calls[].id` on a prior assistant turn (gate numerator); bucket b = field present but null/empty/string-but-no-matching-call (real coverage gap; denominator only); bucket c = field absent entirely (pre-contract turn; excluded from the gate per cross-revision-tolerance). Resolution lookup uses a forward-pass `Map<tool_call_id, assistant_idx>` (O(N) not O(N²)). 9-path first-match-wins dispatch inferer walks existing runtime flags without adding a new emission-site field (that *is* the cross-revision-tolerance design): `refused-envelope` (`_refused: true`), `partial-envelope` (`_partial: true` — recognized for forward-compat; no current emitter), `cross-request-cache-hit` (`_cached: true` + `_cache_note` matches cross-request marker), `same-request-cache-hit` (`_cached: true` without marker), `tier0-sandbox` (`_tier0: true` — no current emitter), `mcp-bridged` (name prefix `mcp__`), `sub-agent` (name `delegate_task`), `plan-mode-post-approval` (prior turn within 5-back has `plan_mode_exit: true` or `metadata.plan_approved === true`), `direct` (fallback). Non-JSON content falls through to `'direct'` (defensive — never throws). Nested envelopes resolve by outermost flag (a `_cached` wrapper around a `_refused` payload reads as cached). Historical-path flagging via the frozen `CURRENT_DISPATCH_PATHS` set + maintainer doc-comment ("when retiring a path emitter, remove its name") — browser-resident probes can't grep `js/` at runtime, so the frozen constant is the lightest cross-revision-tolerant signal compatible with the no-build constraint. Observed paths absent from the set get `is_historical: true` on their `by_path` row and are excluded from the gate roll-up. Gate computation: `pct = passing / eligible * 100` where `eligible = a + b` (c-bucket + historical-path rows excluded); `passes = pct >= threshold` with `threshold` defaulting to 95 (overridable via `opts.threshold` for testing); `pct: null` when `eligible === 0`. Markdown formatter has stable header (`## Rule 3/4 coverage probe`) + summary + gate verdict (`PASSES` / `FAILS` / `no eligible turns`) + per-path table + detected-paths + optional historical-paths warning — stable enough for paste-into-issue flows and snapshot pinning. JSON formatter sorts top-level keys for byte-stable output across two runs on the same history. Button wired into Logs tab of [`js/debug-slideout.js`](../js/debug-slideout.js) via the existing `setTimeout(...,0)` chip-handler idiom — dynamic-imports the probe (lazy load), runs against `State.chatHistory`, stashes markdown + JSON in module-locals, re-renders with `<pre>` output block + `Copy JSON` / `Clear` follow-up chips. **Not** an extension of `metadata-probe.js` (122-LOC frozen-shape presence counter; pinned by `tests/test-metadata-coverage.mjs`; folding both into one file would invert the existing test set). 37 subtests in `tests/test-rule3-coverage-probe.mjs` pin gate computation + dispatch-path inference + historical-path detection + formatter stability. **Rule 3 / Rule 4 deferrals stay open** — the probe is the prerequisite measurement infrastructure, not the ship of Rule 3 itself.
...
**Plan-Mode approved-plan passive-push (2.77.0).** Active-pull via `read_approved_plan` depended on the model choosing to call it; under aging-context pressure the model instead reflexively re-invoked the visible `submit_plan_for_approval` tool (gitea#478 trace: two substantially-identical plans auto-approved in one session, both submissions wasted user attention). `buildSystemPrompt` now appends an `<approved-plan-current>` block (prepended before the `PLAN MODE ACTIVE` block when both are active — the resubmit bug fires *after* approval lifts plan mode, so the new block must survive that lift) whenever `getApprovedPlan()` is non-null. The block wraps the original plan body verbatim with an ISO `approvedAt` attribute + an anti-resubmit nudge. Slot still cleared on conversation switch via the existing `clearApprovedPlan()` lifecycle. `read_approved_plan` retained for one release cycle as a redundant active-pull path; removal candidate once the passive-push soaks.
...
**Plan-mode tool-gate at dispatch boundary (2.76.0; gitea#480).** Pre-2.76.0 plan-mode admission was list-side only — `applyPlanModeFilter` in `js/llm/api.js` scrubbed the LLM-visible tool list using the opt-in `readOnly: true` flag. `write_file` and `create_pull_request` slipped through in a real session (qwen-3-6-plus, HTML-Games issue #232) — they correctly weren't in the list, but `ToolRegistry.executeWithProfile` had no second check, so calls reaching dispatch via cached tool messages / sub-agent loops / plugin shims / stale assistant-turn context ran the handler with no plan-mode awareness. Two-layer defense at 2.76.0: (1) list-side filter migrated from `readOnly` to `side_effects` classification so list-side and dispatch-side share one source of truth; (2) new authoritative `ToolRegistry.checkPlanModeAccess(name)` runs at the top of `executeWithProfile` **before** the role check, admits `side_effects === 'read'` plus the `PLAN_MODE_SESSION_WRITE_ALLOWLIST` (session-local writes — `scratchpad_write`, `todo_write`, the five preview action tools), fail-closed default catches unclassified tools incl. MCP-bridged. New `js/intelligence/tools/side-effects.js` extracted from `catalog.js` to break the registry↔catalog cycle, lifts `SIDE_EFFECTS_BY_NAME` + `getSideEffectByName` out as a cycle-free source of truth; 16 missing tool classifications added (`todo_*`, `delegate_task`, `git_log`, `wait_for_ci`, `submit_script_for_approval`, the 7 preview observation + 5 preview action tools). New `tests/test-plan-mode-source-scan.mjs` catalog-parity lint walks `js/tools/*.js`, extracts every `register('NAME', ...)` literal, asserts each name has a `SIDE_EFFECTS_BY_NAME` entry + bidirectional pin against stale catalog entries. The `readOnly` field on tool definitions stays as a documentation axis but no longer gates behavior.

**T1 tool-authored failure shape conformance (2.78.0).** Every tool-side rejection in the 11-handler `T1_CONFORMANT_TOOLS` cohort carries a `code:` field from a `VALID_CODES` closed set alongside the existing human-readable `error:` narration (`edit_file`, `write_file`, `read_lines`, `search_in_files`, `get_ci_status`, `wait_for_ci`, `get_ci_logs`, `read_plugin_source`, `write_plugin_source`, `run_plugin`, `run_code`). The `code:` field aligns tool-throw failures (the registry catch block at `js/tools/registry.js` already surfaces `EditorError` throws as `{error: msg, code: identifier}`) with tool-return failures on one envelope shape, pinned in [`docs/DESIGN-tools.md`](DESIGN-tools.md) §"Tool-authored failure shape contract" — the ai-editor concrete implementation of the design's "`error` (or `failure_code`)" clause. Recovery-sufficient payload additions accompanied conformance: `edit_file` stale-line returns gain `suggested_start_line` / `suggested_end_line` / `current_content_window` as named fields (previously embedded in narration text only); `read_lines` invalid-line cases gain `actual_file_line_count`. `run_plugin`'s hybrid `{success: false}` envelope normalized per [`docs/DESIGN-agent-loop.md`](DESIGN-agent-loop.md) §"Envelope Shapes" — envelope authorship is the loop's, not the tool's. New `tests/test-tool-failure-shapes.mjs` (~340 LOC) source-scan lint walks `js/tools/*.js`, resolves each `register('NAME', HANDLER, ...)` to its handler body (handles both inline-arrow and named-function-reference patterns — the latter is the `ci-tools.js` shape), asserts every `return {... error: ...}` site in a `T1_CONFORMANT_TOOLS` handler also carries a `code:` field from `VALID_CODES`; 3 subtests. Closed code set: `stale_lines`, `schema_validation_failed`, `precondition_not_met`, `path_not_found`, `read_error`, `write_error`, `search_error`, `search_truncated` (added 2.81.0), `uncommitted_changes` (added 2.84.0), `ci_status_fetch_error`, `ci_workflow_error`, `ci_workflow_not_found`, `ci_log_not_available`, `plugin_execution_error`, `code_execution_error`, `edit_error`, `editor_open_failed`. Subsequent additions land their codes here without sweeping the other handlers' return sites (2.81.0 added `search_truncated` to `VALID_CODES` without graduating `search_in_files` to `T1_CONFORMANT_TOOLS`; 2.84.0 added `uncommitted_changes` for `create_pull_request` likewise). The closed-code-set lint is now a settled idiom alongside capabilities-precedent + turn-stats-precedent + source-scan-precedent — the gate is set-membership rather than per-site shape.

**Session-scoped auto-commit tracker (2.80.0; gitea#486).** `write_file`'s new-file branch calls `Git.createFile()` — a one-file Git commit, separate from any subsequent `commit_files` call. The file enters the tree, **not** as a dirty tab. When `commit_files` later filtered `State.openTabs` by `t.dirty`, it only saw modifications (write_file-on-existing + edit_file paths leave a dirty tab); the response reported only the dirty paths. The model — operating off the `commit_files` response, not its tool history — re-opened the new files via `read_file`, looped `list_dirty_files`, ultimately opened the PR on faith (5+ wasted turns in the gitea#486 field session). New `js/tools/_session-auto-commits.js` (~30 LOC) keeps a session-scoped `Set<string>` with three exports: `recordAutoCommit(path)` idempotent, `getAutoCommittedSinceLastReport()` returns array + drains the set, `clearAutoCommitted()` full clear. `write_file` new-file branch records the path after `Git.createFile` succeeds + response gains `committed: true` + `commit_message: 'Create ${path}'` so the model has the signal at write-time. `commit_files` drains the tracker into `response.created` on success AND on the "no dirty files" early return (so pure-new-file sessions still see the signal). Lifecycle paired with `clearApprovedPlan()` at three `js/chat/conversations.js` sites (conversation switch, new chat, conversation delete). Separate `created` field rather than folding into `committed` because the two arrays describe different events (`committed` = what THIS call flushed; `created` = what `write_file` auto-committed earlier in the session).
...
Implementation under `js/` derives from DESIGN/ICD contracts or from a narrow,
evidence-backed roadmap outcome. Tests verify implementation against contracts;
observed runtime evidence is the authority for promoting architectural repairs.
...
GitHub `Validation` runs on every pull request and `main` push with read-only
repository permission:

1. `Node and policy` checks version coherence, raw-HTML bypasses, invisible
   Unicode, theme tokens, the complete Node suite, the npm lockfile, and a
   moderate-or-higher dependency audit.
2. `Container` builds from digest-pinned bases and locked vendor dependencies,
   confirms non-runtime files are absent, and smoke-tests `/` plus `BASE_PATH`.

A separate tag workflow may publish `linux/amd64` GHCR `vX.Y.Z` and `latest`
images with SBOM and provenance. It first proves the tag commit is on `main` and
that both validation checks succeeded on that exact SHA. PR and `main` jobs do
not publish artifacts, receive production secrets, or deploy. Browser-only
tests remain a declared manual limitation.
