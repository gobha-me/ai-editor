...
> Last sync: **2.63.0** (2026-05-17, seventh RE-EVAL slot — `RE-EVAL following 2.58.0`, overdue by 5 minors; doc-only — no version bump, accumulates in [Unreleased]; 3-minor catch-up from the 2.60.0 sync covering: **2.61.0 `server.roles` dead-letter docstring + Settings copy honesty pass** (ICD #6 finding #1; in-place correction with zero behavior change — field/validator/array/IDB-serialize-path/checkbox-group preserved for back-compat with records pre-dating 2.54.0; cross-references the bridge-side regression assertion at `tests/test-mcp-bridge.mjs:259`), **2.62.0 `retrieval:turn-stats` shape contract extracted + pinned via producer seam** (ICD #5 finding (b2); new pure-Node-importable `js/intelligence/retrieval/turn-stats-shape.js` + `_emitTurnStats` seam at producer + validator-at-entry at consumer; seam-vs-JSDOM architecture decision resolved as seam — aligns with `manager-helpers.js` precedent + no-`package.json` constraint; 18 subtests), and **2.63.0 MCP public-surface shape pins** (ICD #6 finding #3 + §"Open invariants" bullets 1–3; new `tests/test-mcp-public-surface-shape.mjs` with 17 subtests pins `Object.keys(...).sort()` on `js/mcp/{bridge,registry,protocol}.js` plus live/persisted record schemas + naming invariants + `testConnection` envelope + `VALID_TRANSPORTS` coercion; capabilities-precedent direct deepEqual idiom — zero production-file edits). Two precedent shape-pinning idioms now stand as settled patterns: **capabilities-precedent** (direct `Object.keys` deepEqual; 2.50.0 + 2.63.0) for public-export pins, **turn-stats-precedent** (validator-at-a-seam; 2.62.0) for EventBus-coupled payload contracts. Per-subsystem detail lives in [`docs/DESIGN-*.md`](.) and [`docs/ICD-*.md`](.) — seventh ICD authored this slot at [`docs/ICD-plugin-lifecycle.md`](ICD-plugin-lifecycle.md) (plugin in-process registry + external/user-plugin load paths + auto-profile-switch + 5-tool plugin-dev cohort, 5 axes: Installation / Registration / Enablement / Invocation / Cleanup); this doc tracks structural shape only.
...
│   └── ICD-plugin-lifecycle.md       — Plugin in-process registry + external/user load paths + auto-profile-switch + 5-tool plugin-dev cohort; target #7 (RE-EVAL following 2.58.0)
│
