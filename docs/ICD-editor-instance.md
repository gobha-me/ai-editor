...
### Finding #1 [medium] — Per-instance Compartments + `editorInstance` lack a unit-test on the destroy-then-replace contract

The lifecycle invariant — `createEditor` destroys the prior instance + the prior `lineNumberCompartment` / `keymapCompartment` become unreachable + the next `createEditor` reads fresh Compartments — has no regression test. The contract is hand-asserted via two-line code comments at [`instance.js:58–61`](../js/editor/instance.js); a future contributor refactoring the destroy path could silently break it (e.g., by moving the Compartment locals into module scope and forgetting to null them on destroy, which would persist stale Compartment refs across replacements).

**Fix shape:** a new `tests/test-editor-lifecycle.mjs` source-scan test (no JSDOM required, mirroring [`tests/test-plugin-editor-auto-switch-retired.mjs`](../tests/test-plugin-editor-auto-switch-retired.mjs)) that pins:
- `editorInstance.destroy()` is called inside `createEditor` before `editorInstance = new CM.EditorView(...)` (positive grep on the order).
- `lineNumberCompartment` + `keymapCompartment` are assigned inside `createEditor` (function-local), not at module scope (negative grep at module-scope `let`).
- A future PR that promotes either Compartment to module scope must also add a null-on-destroy hook + update this test.

Pre-emptive — no live bug, but the destroy-then-replace contract is the most easily-broken invariant in the editor module. **Sizing:** [S] (single test file, <80 LOC; zero production-file edits).
