...
|---|---|---|
| **`notSupported` throw** (the rule) | 40 of 55 methods — listRepos, createRepo, getRepo, listBranches, createBranch, deleteBranch, getContents, getFileTree, getFile, getBlame, getFileCommits, getCommits, createFile, updateFile, deleteFile, renameFile, batchCommitFiles, all 7 issues methods, all PR methods except `addPullRequestComment`/`getMergeConflicts`, `submitPullRequestReview`, `createReviewComment`, `resolveReviewThread`, `getCommitStatus`, `listTags`, `compareRefs`, `listReleases`, `createRelease`, `rerunWorkflowJobs`, `downloadArchive` | Default: caller MUST handle "this provider can't do this" — surfaced through `EditorError(GIT_NOT_SUPPORTED)` with a friendly hint |
...
| `threadResolve` | **declared but not yet read** (1 doc-ref at `base.js:582` documenting the dead-code state) | Would enable Resolve Thread buttons on review threads; no provider implements `resolveReviewThread` — see code-aware finding #3 |
...
### 3. `resolveReviewThread` is dead code today
...
**Suggested fix shape (queued, not promoted):** Two options. (a) Promote: when a provider grows the capability (e.g. GitHub GraphQL integration ships), add the override + flip the flag + wire the dock button. (b) Demote: if the consensus is that thread resolution lives in a separate UX path (local-only via `review-state.markResolvedLocal()`), remove the base method entirely + its capability flag. **Decision deferred** — the dead-code state is documented and harmless; the deletion is a real code change that needs its own slice.
...
- **Design contracts:** [`docs/DESIGN-git-providers-and-ui-extensions.md`](DESIGN-git-providers-and-ui-extensions.md) — historical-shape 1.1.0 minimum-viable surface; this ICD is the current-shape source of truth. [`docs/ROLES_AND_TOOLS.md`](ROLES_AND_TOOLS.md) — provider-contributed tool admission. [`docs/SECURITY.md`](SECURITY.md) — untrusted-issue-content wrap markers (`<UNTRUSTED_*>` from issue/PR bodies pass through provider methods unchanged).
