...
#### ~~[REG] [M] [likely] Git-provider `panels` manifest still flat-slot, not rail-views~~ *(✅ closed — shipped 2.43.0)*
- **What:** Every git provider (`js/git-providers/gitea.js:1177-1197`, `github.js:1199-1219`, `gitlab.js:~1170`) declared `panels: [{slot: 'sidebar-panels', ...}]` with no `render` and no `view:` shape. `local.js:278` had empty arrays.
- **Why it's load-bearing:** Per the 2.22.0/2.23.0 decision, rail-views is the structured slot; the 6 declarations were dead metadata. The intended migration was to make rail-views the seam — either by flipping these to `slot: 'rail-views'` with `view: {id, label, icon, badge, priority}` so providers ship their own rail views (e.g. GitHub Actions panel, GitLab Pipelines panel) without core code changes, or by deleting the dead entries and confirming the seam is already wired.
...
- **Resolution:** 2.43.0 deleted the 6 dead entries (gitea-issues, gitea-prs, github-issues, github-prs, gitlab-issues, gitlab-mrs) from providers' `contributes.panels` arrays. The `rail-views` seam is exercised by `tests/test-slot-manager.mjs` (`rail-views: applyProviderContributions wires structured panels from provider manifests`) which builds a synthetic provider, calls `applyProviderContributions()`, and asserts the entry lands in `getContributions('rail-views')`; built-in views remain the provider-agnostic source of truth in `js/ui/left-pane-rail.js#BUILTIN_VIEWS`. New regression test `providers ship no flat-slot panels metadata (2.43.0 manifest prune)` prevents flat-slot reintroduction. Docstring at `js/slot-manager.js` header + inline comment at `js/app.js:723` rewritten to reflect post-migration state. **Recommendation interpretation:** the audit text said "convert", but converting six entries to view.id's that already exist as built-ins (`issues`, `prs`) yields collision-skipped no-ops — accumulation, not admissibility. Deletion mirrors prior closures (2.24.0, 2.33.0, 2.41.0) that replaced parallel rebuilds with single sources of truth. See CHANGELOG §2.43.0.
...
#### ~~[EV] [S] [maybe-intentional] `git:branchCreated` has 3 emitters, 0 subscribers~~ *(✅ closed — shipped 2.39.0.1)*
...
#### ~~[HC] [M] [needs-investigation] Tool-name string-literals dotted around chat module~~ *(✅ closed — shipped 2.44.0.0)*
...
- **Resolution:** 2.44.0.0 (sweep wave slice 1) — investigation rejected pure centralization (a single `TOOL_NAMES` constant): the `switch (toolName) { case 'X': ... }` idiom in `summarizeToolArgs` / `summarizeToolResult` is more readable as bare literals than as `case TOOL_NAMES.read_lines:`, and the equality checks in `turn-enrich.js` would gain no compile-time safety from indirection. Shipped coverage instead — the established pattern of `tests/test-no-raw-localstorage.mjs` (2.40.0) and `tests/test-no-inline-onclick.mjs` (2.32.0): glob source, regex-extract, cross-reference against the canonical registered-tool set. **Minimal hoist:** `requiredParams` in `js/chat/tools.js` lifted from function-local `const` inside `validateToolParameters` to module-scope `export const REQUIRED_TOOL_PARAMS = Object.freeze({...})` (function body now reads from it; behavior byte-equivalent) so the new test can `import` and cross-reference its keys. **New test `tests/test-chat-tool-name-literals.mjs` (4 cases):** Case A — `REQUIRED_TOOL_PARAMS` parity against the canonical set; Case B — `messages.js` `summarizeToolArgs` / `summarizeToolResult` `case 'X':` labels parity (body-walk via brace-counting); Case C — `turn-enrich.js` `toolName === 'X'` and `toolName.startsWith('X')` equality literals parity; Case D — sanity that the scanner returns ≥ 30 names and includes a stable spot-check handful. Source-of-truth scanner reads `js/tools/*.js` for `(?:ToolRegistry|registry)\.register\('NAME'`, capturing both top-level `ToolRegistry.register(...)` side-effect registrations (commit-tools / ci-tools / context-tools / git-log-tools) and the `registry.register(...)` shadowed-arg form inside the `register<X>Tools(registry)` helpers (file-tools / scan-tools / edit-tools / etc.). One allow-list entry: `peek_read_lines` (meta-tool layered above `read_lines`, not a directly-registered tool). See [CHANGELOG §2.44.0.0](../../CHANGELOG.md).
...
#### ~~[EV] [S] [likely] `git:fileLoaded`, `git:loadingFile`, `git:saving`, `git:folderDeleted`, `git:folderRenamed`, `git:repoCreated`, `git:issueCreated`, `git:issueCommented`, `git:mrCreated`, `git:prMerged`, `git:prReviewSubmitted`, `git:ciRerun`, `git:batchSaving` all emit with 0 internal subscribers~~ *(✅ closed — shipped 2.39.0.1)*
...
#### ~~[ST] [L] [likely] 53 inline `onclick="window.foo()"` calls across `html/*.html`~~ *(✅ closed — full 4-phase arc shipped 2.27.0 → 2.32.0)*
...
- **Resolution:** Designed at [`docs/DESIGN-html-inline-handlers-migration.md`](../DESIGN-html-inline-handlers-migration.md); 4-phase rollout shipped 2.27.0 → 2.32.0. Phase 1 (pilot commit modal, 2.27.0), Phase 2a (8 modals, 2.28.0), Phase 2b (3 remaining modals + app-shell, 2.29.0), Phase 3a (7 JS renderers — diff-viewer / file-tree / issue-list / tab-manager / chat-input / pr-list / issue-detail, 2.30.0), Phase 3b (`js/chat/messages.js` — 13 handlers including `this`-passing copy/edit and the Decision-5 `classList.toggle`, 2.31.0), Phase 4 (`window.*` exposure block cleanup — 56 dead aliases retired, 15 kept with cited consumers, new `tests/test-no-inline-onclick.mjs` anti-regression coverage, 2.32.0). The 5 retained aliases tied to non-`onclick` inline handlers (`pinTab` / `openIssueTab` / `openPrReview` / `handleZipFileSelect` / `zipToggleFile`) cover the residual `ondblclick=` / `onchange=` / `onkeydown=` strings deliberately out of Phase 3's scope; a future patch can convert those residuals + retire each alias independently.
