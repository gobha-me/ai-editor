...
**Phase 2 — Parallel sub-agents.** Same `subagent.v1` profile; the parent agent issues N `delegate_task` calls in a single round (the standard provider tool-use API admits multiple `tool_use` blocks per assistant turn). Each runs in its own `SubAgentContext`; N `tool_result` blocks emit back to the parent in the same next round, preserving the model's 1-to-1 `tool_use`-to-`tool_result` mapping.

*Implementation-gated on the Phase 1 dogfood-signal definition below. This sub-section is the spec the implementation PR will follow; it does **not** open the slot.*

**Implementation pinned by this spec pass (post-Phase 1 OQ resolutions):**

| Sub-decision | Resolution | Rationale |
|---|---|---|
| Approval-card UX | **Single batched card with per-row Approve/Reject/Cancel + "Approve all"/"Reject all" footer.** Capability summary lists the union of admitted tools across all N rows, plus a per-row collapsed view of any row-specific narrowing (`args.tools` intersection). | Mirrors the file-list shape `commit_files` and `script-approval-card` already use. N independent cards stack the chat surface and create per-card watchdog races; one batched card means one `USER_PAUSE_TOOLS` watchdog covers the whole batch. |
| In-flight approval state | **`pendingSubAgentApprovalBatch: { id, slots: Map<callId, ApprovalSlot> } \| null`** replaces Phase 1's single-slot `pendingSubAgentApproval`. Single batch in flight at a time (parent loop pauses on the batch); slots inside the batch resolve concurrently after Approve. | Mirrors how `script-approval-card` already handles a queue of files within one approval. Single-batch-in-flight preserves Phase 1's `USER_PAUSE_TOOLS` contract; per-call concurrency is the new lift. |
| `session_cost` race mitigation | **New `subagentCostMutex` keyed by `'session'`**, copied from the `KeyMutex` precedent in [`js/intelligence/cost/cost-store.js`](../js/intelligence/cost/cost-store.js) `recordTurn`. Both the per-sub-agent finalizer (cost increment) and the per-call pre-flight (`session_cost.dollars + estimated_cost > subagentSessionCap?`) go through it. | Phase 1 doesn't need it (one in-flight at a time). Phase 2 ships with it as a precondition. No new mutex primitive — the existing `KeyMutex` is the call-site shape. |
| `LLMDebug` per-loop view | **Debug-modal grouping learns to render parallel siblings side-by-side** (columns) rather than serial rows. Phase 1 already threads `subagent_id` into diagnostic entries (per Decision §8); Phase 2 needs no new fields, just a UI grouping change scoped to this PR. | Not a separate slot. The data shape is already in place; only the renderer needs to learn the layout. |
| Per-call `tools` allowlist strictness | **Stay lenient** for Phase 2's first ship (silent drop with `LLMDebug` note, same as Phase 1). Strict-mode promotion stays gated on dogfood evidence of typo-driven admission drops. | Parallel calls do not change the per-call admission contract; the typo-vs-strict tension is orthogonal to phase count. Strict-mode change earns its own slot whenever the evidence triggers it. |

**Parent-loop integration (new contract):**

1. When the parent model emits multiple `delegate_task` `tool_use` blocks in one round, the parent loop collects them into one `pendingSubAgentApprovalBatch` rather than dispatching the first and pausing on it.
2. The approval card mounts once with N rows.
3. On Approve, each row's sub-agent loop starts concurrently (`Promise.all` over the runner array). Each runner is the same Phase 1 sub-agent loop, unchanged.
4. The parent loop awaits the batch-resolution Promise, which resolves when all N reach a terminal state (`completed` / `errored` / `cancelled` / `rejected` / `partial`).
5. N `tool_result` blocks emit back to the parent in the same next round.

**This pins the contract: parallel sub-agents do not cause out-of-order `tool_result` delivery to the parent.** If a fast sub-agent finishes first, its result still waits for the slowest before the batch returns. The alternative (stream results as they finish across multiple parent turns) would require provider-level support for multi-turn `tool_result` streaming, which Venice/Anthropic do not offer today. If that capability ever lands, the streaming path becomes a follow-on sub-section here; the batched-await is the Phase 2 ship.

**State-shape touchpoints:**

- [`js/chat/state.js`](../js/chat/state.js): replace `pendingSubAgentApproval` with `pendingSubAgentApprovalBatch`; rename helper pair (`setPendingSubAgentApprovalBatch` / `cancelSubAgentApprovalBatch`); add per-row helpers (`approveSubAgentSlot(callId)` / `rejectSubAgentSlot(callId, feedback)` / `cancelSubAgentSlot(callId)`).
- [`js/chat/subagent-approval-card/SubAgentApprovalCard.js`](../js/chat/subagent-approval-card/SubAgentApprovalCard.js): same component; gains a per-row sub-renderer + footer actions. No new component file.
- Sub-agent finalizer in the tool-loop core: wraps the `State.subagents.session_cost` read-modify-write in `subagentCostMutex.run('session', async () => …)`.
- [`js/chat/tool-loop-core.js`](../js/chat/tool-loop-core.js): no changes to the loop body; the per-runner construction call-site (one per row) is the only edit.

**Sizing:** ~6 files net change; ~400–600 LOC. Smaller than Phase 1 because no new tool, no new profile, no new system prompt addendum, no new persistence shape (per-call transcripts already use a keyed Map). Same shape as the 2.49.0 → 2.49.0.0 slice pattern (plumbing slice + UX slice could split if needed).

**Gating: still implementation-gated on Phase 1 dogfood evidence.** See §"Phase 1 dogfood-signal definition" below. This spec describes the shape; the trigger to build it is empirical, not calendrical.

**Phase 1 dogfood-signal definition** (Phase 2 implementation gate)

Concrete falsifiable conditions that, once met, promote Phase 2 from "spec'd" to `[strong]`-band ready-to-build. Any one of A/B/C fires the implementation slot:

- **Trigger A — Serial-dispatch evidence:** ≥2 distinct user sessions show the parent agent making 3+ consecutive `delegate_task` calls in the same round-trip series (serial dispatch where parallel would have saved wall-clock). Measurable from the cost store's per-conversation per-tool timeline ([`js/intelligence/cost/cost-store.js`](../js/intelligence/cost/cost-store.js) `getTurnRowsForConversation` or equivalent) — count of adjacent `tool_name: 'delegate_task'` rows in the same conversation within a 10-minute window.
- **Trigger B — Model intent evidence:** ≥1 session where the model's response prompt or scratchpad output explicitly references wanting parallelism (e.g., "I would dispatch these in parallel if I could", "ideally these reads run concurrently"). Caught via post-session AAR rather than instrumentation.
- **Trigger C — User-reported pain:** explicit ask for parallel sub-agents, either inline in a session or as a ticket comment. Counts as 1 dogfood signal.

None of A/B/C is satisfied as of this spec pass (Phase 1 shipped 2026-05-14, 2 days before this spec; insufficient corpus). Re-check at every `RE-EVAL following X.Y.Z` slot per [`docs/ROADMAP.md`](ROADMAP.md) §"Re-evaluation cadence"; promote when the first trigger fires.
...
| **Concurrent sub-agents (Phase 2) race on `State.subagents.session_cost`.** Two sub-agents reading-and-incrementing the cost slot simultaneously. | Medium for Phase 2. | **Pinned by §Phasing → Phase 2 spec:** new `subagentCostMutex` keyed by `'session'`, copying the `KeyMutex` call-site shape from [`js/intelligence/cost/cost-store.js`](../js/intelligence/cost/cost-store.js) `recordTurn`. Both the per-sub-agent finalizer and the per-call pre-flight check go through it. Phase 1 (single sub-agent) does not need it; Phase 2 ships with it as a precondition. |
...
*Three Phase-2-specific OQs (batched-vs-N approval cards; `LLMDebug` per-loop partition; per-call `tools` allowlist strictness) were resolved in the Phase 2 spec pass — see §"Phasing → Phase 2" sub-decisions table above.*
...
| Two `delegate_task` calls in flight (Phase 1) | The single-slot guard in `state.js` (`pendingSubAgentApproval`) rejects the second with a console warn | Cannot happen in Phase 1 (the chat loop pauses on the first card). If it does, it is a bug. **Phase 2 lifts this** by replacing the single-slot with `pendingSubAgentApprovalBatch` — see §Phasing → Phase 2 sub-decisions table for the new state shape. |
...
**Later (gated on Phase 1 dogfood):** *Phases 2–6.* Each gated by named, falsifiable conditions in *Phasing* above. **Phase 2 (parallel) — fully spec'd 2026-05-16 in §Phasing → Phase 2 above; implementation slot still gated on §"Phase 1 dogfood-signal definition" Triggers A/B/C.** Phase 3 (recursive) gated on two-level-decomposition-outperforms-single-level evidence. Phase 4 (auto-approve) gated on a corpus of low-risk low-cost approvals where the click is friction. Phase 5 (cross-session persistence) gated on a consent design. Phase 6 (background sub-agents) gated on Window v2 Sessions. **No row in the Roadmap until a trigger fires.** Speculative slotting now would lock in conclusions the design cannot yet justify.
