...
| Phase 3b | `js/chat/messages.js` alone (~13 handlers — `this`-passing + DOM-only `classList.toggle` per Decision 5) | shipped | 2.31.0 | `mountChatMessages` in [`js/chat/messages.js`](../js/chat/messages.js) scoped by `#chatMessages` — 9 callback actions + the Decision-5 generic `toggleExpanded` (reads `data-target`, calls `getElementById(target)?.classList.toggle('expanded')`). `this`-passing copy/edit/commitEdit/cancelEdit shapes survive by passing the matched `btn` element to the callback (existing `buttonEl.closest('.chat-message')` traversals unchanged). `previewImage` reads `data-src` since `<img>` carried `this.src`. New `tests/test-chat-messages-dispatch.mjs` (16 cases). `window.Chat.*` aliases stay through Phase 4. HTML side of the migration now complete. |
