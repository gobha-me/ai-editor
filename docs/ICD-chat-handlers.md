...
> **📎 Post-supersedence notes — agent-loop surface naming (2026-05-21, eleventh re-eval slot).** Three contracts that ship under `js/chat/` since the original 2.42.0 authoring of this ICD are now named at [`docs/DESIGN-agent-loop.md`](DESIGN-agent-loop.md) (landed 2026-05-21) + cited at the new JSDoc-only module [`js/chat/agent-loop-contracts.js`](../js/chat/agent-loop-contracts.js) (2.83.0): (1) the envelope-authorship rule that distinguishes loop-authored fields (`_refused`, `_cached`, `_cache_note`, `next_action_hint`) from tool-authored fields; (2) the `LoopState` typedef enumerating the request-scoped counters (`noProgressStreak`, `roundCount`, `duplicateStreak`, `sameToolStreak` 2.86.0, `lastInvokedToolName` 2.86.0, `toolCallCache`, `toolActionLog`, `queuedInputFIFO`, `userPausePromise`); (3) the `EnvelopeShape` union over `SuccessEnvelope` / `RefusedEnvelope` / `CachedEnvelope` / `PartialEnvelope` (flag-presence discrimination; no `kind` field added to runtime envelopes). **The Authorship Rule classification table lives at the new module verbatim from the design doc — this ICD references but does not duplicate.** Same-tool-name streak guard (`SAME_TOOL_REFUSE_THRESHOLD = 5`, args-independent, OR-gated with the args-exact `DUP_REFUSE_THRESHOLD = 3`; gitea#496 at 2.86.0) is the second arm of the dup-streak axis — see also the consumer paragraphs §"Five axes" (Cache(dup)) below. Rule 3/4 coverage measurement is staged via [`js/chat/rule3-coverage-probe.js`](../js/chat/rule3-coverage-probe.js) (2.85.0) — pure-Node-importable probe with 9-path first-match-wins dispatch inferer + 3-bucket classifier + frozen `CURRENT_DISPATCH_PATHS` set for historical-path flagging; the probe IS the measurement infrastructure Rule 3 ships against. **This ICD's scope (the 5 axes + Composer-vs-non-Composer enumeration) stays as-is — the new contracts are documented at their authoritative sites and cross-referenced here.**
...
- Source: [`js/chat/handlers.js`](../js/chat/handlers.js) (tool loop entry), [`js/chat/tool-loop-core.js`](../js/chat/tool-loop-core.js) (extracted reusable loop body; 2.48.0 Phase 0), [`js/chat/tool-classifications.js`](../js/chat/tool-classifications.js) (axis exports), [`js/chat/cache-invalidation.js`](../js/chat/cache-invalidation.js) (eviction helpers), [`js/chat/cache-policy.js`](../js/chat/cache-policy.js) (`isStatefulRead` / `getStatefulReadToolsLive` for the stateful-read bypass), [`js/chat/refusal-hints.js`](../js/chat/refusal-hints.js) (`buildRefusalPayload` authors `RefusedEnvelope`), [`js/chat/agent-loop-contracts.js`](../js/chat/agent-loop-contracts.js) (2.83.0; JSDoc-only typedefs + Authorship Rule classification table verbatim from the design doc), [`js/chat/rule3-coverage-probe.js`](../js/chat/rule3-coverage-probe.js) (2.85.0; pure-Node Rule 3/4 coverage measurement), [`js/chat/turn-enrich.js`](../js/chat/turn-enrich.js) (FileOp axis consumer), [`js/prompts.js`](../js/prompts.js) (Composer-vs-non-Composer enumeration), [`js/llm/api.js`](../js/llm/api.js) (`LLMTools.getAdmittedTools` / `getToolsForRole`).
- Tests: [`tests/test-tool-classifications.mjs`](../tests/test-tool-classifications.mjs) (disjointness + frozen-array invariants), [`tests/test-system-prompt-admission.mjs`](../tests/test-system-prompt-admission.mjs) (2.35.0 Composer-vs-non-Composer projection), [`tests/test-agent-loop-contracts-citation.mjs`](../tests/test-agent-loop-contracts-citation.mjs) (2.83.0; 7 subtests — 4 consumer citation lints + 3 contracts-module self-checks), [`tests/test-tool-loop-anti-loop.mjs`](../tests/test-tool-loop-anti-loop.mjs) (2.86.0; 8 subtests pinning the OR-gated dup-streak guard semantics), [`tests/test-rule3-coverage-probe.mjs`](../tests/test-rule3-coverage-probe.mjs) (2.85.0; 37 subtests).
- Design contracts: [`docs/DESIGN-agent-loop.md`](DESIGN-agent-loop.md) (2026-05-21; names the previously-unnamed agent-loop consumer surface), [`docs/DESIGN-tools.md`](DESIGN-tools.md) §"Tool-authored failure shape contract" (2.78.0; cross-references the Authorship Rule for envelope ownership), [`docs/DESIGN-INDEX.md`](DESIGN-INDEX.md) (8-doc index landed alongside the 2026-05-21 intelligence-layer pack).
- History anchors: 2.25.0 hoist (audit-2026-Q2 inventory [DUP][M]); 2.35.0 `LEGACY_TOOL_ENUMERATION` deletion; 1.7.1 / gitea#301 (cache-invalidation pattern instance 1); 2.10.0 / Tier 3a + github#39 (pattern instance 2); 2.10.1 (cross-request entry args-matching fix); github#35 (envelope axis introduction); 2.78.0 / T1 conformance (tool-authored failure shape `code:` field + `VALID_CODES` lint); 2.82.0 / gitea#488 (anti-loop refusal `suggestions` field removed — bad suggestions worse than no suggestions); 2.83.0 (agent-loop-contracts module + Authorship Rule centralization); 2.85.0 (Rule 3/4 coverage probe — gates the deferred Compression Rules); 2.86.0 / gitea#496 (same-tool-name streak guard OR-gated with args-exact guard — defeats one-byte-arg-variation defeat).
