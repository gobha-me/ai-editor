...
**Phase 3 — Tier 3 sidecar + selector-shaped tools (`snapshot`, `click`, `fill`, `inspect`, `screenshot`, `network`, `resize`).** Full surface parity with the reference MCP minus `preview_eval`. Build-step projects (Cogfall) become previewable. Substantially larger PR (sidecar lifecycle, container isolation, per-workspace dev-server). Gated on Phase 2 producing a use case that selector-shaped tools could serve.

**Phase 4 — `preview_eval` with per-invocation gate, conditionally.** Only if Phase 3 surfaces a measured class of probe that selectors cannot serve. Mirrors the `submit_script_for_approval` approval-card mechanic for the `expression` argument. May never ship.
