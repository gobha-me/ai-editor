...
| **Now** | **Slot reopened.** 1.21.0 just shipped (Settings profile picker — last user-visible slice before the load-bearing flip). The four resolver rewires (compression 1.17.0, memory 1.18.0, tools 1.19.0, retrieval 1.20.0) all in main untagged behind 1.21.0. With the picker shipped, profiles are now user-selectable; default behavior is unchanged (the picker defaults to `(use role)`, threading through `roleToProfileName(role)` byte-for-byte). |
| **Next** | **Path to 2.0.0** — One slice remains: **2.0.0** (role selector retires; migration script + role-gate cleanup at the two coder-gated resolver call sites). Remaining parallel candidates: Plugin Discoverability minor, Retrieval ingest hardening, **In-editor preview & verify Tier 1** (design landed 2026-05-08; see [`docs/DESIGN-preview.md`](DESIGN-preview.md) and §"Parallel 1.X tracks" below). |
