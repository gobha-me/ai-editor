...
| **Now** | **Slice 2 of path-to-2.0.0 (1.24.0) — in flight.** Consumer flip from `Roles.filterTools(defs)` to `Profiles.filterTools(defs, getActiveProfileName(State.settings))` at the two `js/llm/api.js` call sites + the `role.systemPrompt` → `profile.systemPrompt` flip at `js/prompts.js`. `roleToProfileName` widened from `coder ? 'coder.v1' : 'chat.v1'` to a 5-key table mapping every legacy role to its synthetic profile (registered at 1.23.0). Byte-equivalent for every existing user. Field-test for one or more minors before slice 3 (2.0.0) yanks the role selector + runs the migration. |
| **Next** | **Slice 3 of path-to-2.0.0 (2.0.0)** — role selector retires; migration script (`settings.role` → `settings.profile.preset`, audit-logged) + role-gate cleanup at the two coder-gated resolver call sites (tools, retrieval). Parallel 1.X candidates queued behind it: Plugin Discoverability minor, Retrieval ingest hardening (measurement-first), Tier 2/3 of in-editor preview (gated on dogfood signal — Tier 1 shipped at 1.22.0). |
...
|---|---|---|---|
| ~~**1.14.0**~~ *(✅ shipped — PR #322, commit `17c8e35`, tag `v1.14.0`)* | `chat-v1.js` data file; `inheritance.js` `resolveProfile(profile, lookup)` deep-merge with cycle detection; `index.js` barrel. | Profile contract gains a working `base`-chain resolver. **Data only — no consumer reads from a resolved profile yet.** | No. |
| ~~**1.14.1**~~ *(✅ shipped — patch on top of `v1.14.0`)* | `coder-v1.js`: set `base: 'chat.v1'`; trim overrides matching base (`total_tokens`, `system_reserve`, `history_reserve` in `budget`; `chunkers`, `metadata_extensions` in `retrieval`). Keep the actual overrides. New `tests/test-profile-resolution.mjs` proves `resolveProfile(CODER_V1_TRIMMED, lookup) ≡ CODER_V1_PRE_TRIM` field-by-field. | None — data-only. The test is the proof that subsequent slices can rely on resolution being sound. | No. |
| ~~**1.15.0**~~ *(✅ shipped — re-admission markers + capacity cap. The novelty algorithm + Composer step 6.5 already shipped in 1.4.18 with `task_ledger: null` at the production call site; this slice flipped that null on at [`js/intelligence/retrieval/manager.js:651`](../js/intelligence/retrieval/manager.js), pinned the marker format including `; ~<tokens> tokens`, added capacity-cap enforcement in `_spillIfAtCapacity`, filtered marker chunks from `rollupToFiles`, and added `scoreNovelty` as a public alias for `_computeNovelty`.)* | Task ledger Phase 1 — populate `admissions`/`exclusions` arrays at runtime; implement `scoreNovelty(currentQuery, priorAdmission)`; suppression marker `[Already admitted: ...]`. Wires into the retrieval Composer so a follow-up turn that already admitted a chunk gets a marker instead of re-paste. | Retrieval Composer gains a ledger-consultation step before each admission. | **Yes** — follow-up turns that re-touch already-admitted chunks see a marker instead of re-pasted content. Token savings on multi-turn coder workflows. |
| ~~**1.16.0**~~ *(✅ shipped — parallel 1.X track interleaved per §"Parallel 1.X tracks". LLM-authored automation Phase 1: `submit_script_for_approval` tool + Tier-0 in-browser Worker + Plan-Mode-shaped approval card. **Profiles arc slice numbering shifts by 1 from here.**)* | LLM-authored automation Phase 1 (parallel track — does not advance the profiles arc). | None — independent of the profiles arc. | **Yes** — coder gets a new tool-loop pause surface. Chat surfaces unaffected. |
| ~~**1.17.0**~~ *(✅ shipped — was 1.16.0 pre-interleave)* | `resolve.js` rewired: `resolveCompressionConfig(profileName)` (was `(role)`) — returns the resolved profile's compression slice via `resolveProfile`. Coder still gets its rules; chat surfaces now use `chat.v1.compression` (Rule 5 only, **`preserve_recent: 4`** instead of the retired `rule5_only_shim`'s `24`). New `roleToProfileName(role)` translator inside `resolve.js` keeps callers role-keyed until 1.21.0. Long-chat regression at [`tests/test-compression-long-chat.mjs`](../tests/test-compression-long-chat.mjs). | Compression resolver reads from a *resolved* profile (deep-merge consulted at lookup). | **Yes** — chat surfaces (`role !== 'coder'`) drop `preserve_recent` from 24 to 4. Reconciles the divergence noted in [`chat-v1.js:82–89`](../js/profiles/chat-v1.js). |
| ~~**1.18.0**~~ *(✅ shipped — was 1.17.0 pre-interleave; PR #330)* | Memory subsystem reads `profile.memory` via `resolveMemoryConfig(profileName)` — `default_scope`, `propose_after_n_turns`, `capacity_warnings`. `resolveDefaultRememberScope(role)` clamps non-memory-store defaults (coder.v1's `'session'`) to `'workspace'`. | Memory subsystem joins compression on the resolved-profile lookup pattern. | No — clamp preserves pre-1.18.0 behavior for coder; chat surfaces gain a `'user'` default that takes effect when chat surfaces gain memory tools. |
| ~~**1.19.0**~~ *(✅ shipped — was 1.18.0 pre-interleave; PR #332)* | Tools subsystem reads `profile.tools.static` via `resolveTools(profileName)`. Removes the direct `CODER_V1` import at the `recordToolInvocation` / `recordDiscoveryAdmissions` sites in [`js/chat/handlers.js`](../js/chat/handlers.js). | Tools subsystem on the resolved-profile lookup pattern. | No — same static set; just via the resolver. |
| ~~**1.20.0**~~ *(✅ shipped — was 1.19.0 pre-interleave; PR #333)* | Retrieval Composer reads `profile.retrieval` via `resolveRetrievalConfig(profileName)` — `strategy_weights`, `collections`, `memory_collections`, `novelty_threshold` returned for forward-looking parity; today's call site consumes `noveltyThreshold` only. | Last subsystem rewire. **Profiles are now load-bearing internally** even though the Settings surface is still role-keyed. | No — same retrieval behavior per role. |
| ~~**1.21.0**~~ *(✅ shipped — was 1.20.0 pre-interleave)* | Settings UI — profile picker. New [`js/profiles/registry.js`](../js/profiles/registry.js) exposing `Profiles.get` / `has` / `list`. Picker in `Settings → Roles` sits above the role-card grid. New `getActiveProfileName({ profile, role })` helper encodes the precedence: picker wins if set + registered, else falls through to `roleToProfileName(role)`. Compression + memory call sites flipped through the helper; the two coder-gated sites (tools, retrieval) keep their hardcoded `'coder.v1'` until the role gate retires at 2.0.0. | Settings surface gains a parallel control; role↔profile translator carries the duality. | **Yes** — new picker UI visible in Settings → Roles. Default `(use role)` preserves pre-1.21.0 behavior byte-for-byte. |
| ~~**1.23.0**~~ *(✅ shipped — slice 1 of 3 of the 2.0.0 retirement)* | Profile-side admission filter: four synthetic profiles registered (`full.v1`, `plugin-dev.v1`, `pm.v1`, `reviewer.v1` — `Profiles.has`/`get` reachable, excluded from `Profiles.list()`). `Profiles.filterTools(defs, profileName)` mirrors legacy `Roles.filterTools(defs)` byte-for-byte. `chat.v1.tools.allowed_groups: ['all', 'pm', 'reviewer']` (broadened so picker-selected `chat.v1` covers historical pm + reviewer surfaces). `BUILTIN_ROLES['plugin-dev'].systemPrompt` now imports `PLUGIN_DEV_SYSTEM_PROMPT` from `js/profiles/plugin-dev-v1.js` so both `Roles` and `Profiles` share the source of truth. Cross-product equivalence test pins the parallel filter. **No consumer reads from the new helper yet** — slice 1 is data + a parallel filter + tests. | New parallel admission filter alongside legacy. | No. |
| **1.24.0** *(in flight — slice 2 of 3)* | Consumer flip: both `js/llm/api.js` filter call sites (cost baseline, legacy fallback) flipped from `Roles.filterTools(defs)` to `Profiles.filterTools(defs, getActiveProfileName(State.settings))`; `js/prompts.js` SDK addendum flipped from `role.systemPrompt` to `profile.systemPrompt`. `roleToProfileName` widened from `coder ? 'coder.v1' : 'chat.v1'` to a 5-key table mapping every legacy role to its synthetic-or-canonical profile, so the runtime path is byte-equivalent for every existing user. `Roles` import dropped from `js/llm/api.js` (unused after the flip). Slice exit criterion: `node --test` regression suite + the `roleToProfileName` direct-mapping block in `tests/test-resolve-active-profile.mjs`. | Profile-keyed reads now win at the admission-filter + system-prompt sites; `Roles.filterTools` becomes orphaned in `js/core.js` (kept for slice-3 cleanup). | No — byte-equivalent runtime for every existing user. |
| **2.0.0** *(slice 3 of 3 — final)* | Role selector removed. Migration script: `settings.role` → `settings.profile.preset`, audit-logged, one-shot. The `roleToProfileName` translator in `resolve.js` is deleted; consumers get profile name directly. The orphaned `Roles.filterTools` in `js/core.js` is deleted; `BUILTIN_ROLES` retires; the synthetic profiles' `Profiles.list()` exclusion can stay or relax depending on the picker UX direction. Per-profile diagnostics surfaces show active profile name + version. | **The profile contract is the load-bearing configuration surface.** Role concept is gone from the UI and from the lookup paths. | **Yes** — role selector disappears. Existing users get migrated. By design, behavior is identical for any user who never touched the picker. Unblocks github#24 sub-agents and Touch 3 Window v2 / Sessions. |
...
> **Plugin Discoverability — single feature minor.** Closes the three "⚠️ Works But No Settings UI" rows from [`docs/PLUGIN.md`](PLUGIN.md) lines 429–437 (rendered as the "Plugin SDK" help page): LLM-provider dropdown reads `Providers.list()` instead of a hardcoded list; git-provider dropdown reads `GitProviderRegistry.list()`; new "Plugin Tools" subsection in [`js/settings/plugins-tab.js`](../js/settings/plugins-tab.js) iterating tools whose registry source is a plugin. Removes the matching parking-lot row ("Dynamic provider registration in settings UI") from §"Migrated from retired docs/PLAN.md". The four "❌ Not Currently Possible" items stay parked — Settings-panel tabs and DOM-slot injection are gated on the unbuilt `SlotManager` (already in §"Other deferred"); Tool-config UI and CodeMirror bridge are larger separate minors.

> **In-editor preview & verify (Tier 1) — single feature minor.** Closes the platform-level gap surfaced by the 2026-05-08 Sokoban dogfood incident on HTML-Games (`xcaliber/HTML-Games` PR #170 shipped a Sokoban whose `bindEvents()` never ran because `updateUI()` threw on a missing `#level-display`; every key was a no-op end-to-end). The agent had no surface to load the page in a browser, so it could not observe the failure. Tier 1 = sandboxed iframe at a distinct preview origin (or `sandbox="allow-scripts"` no-`allow-same-origin` for self-host) + Service Worker resolving workspace paths via the existing `Git.getFile` adapter + three new tools (`preview_start`, `preview_stop`, `preview_list`) registered in `js/tools/preview-tools.js`, all `readOnly: true`. Build-step projects (Cogfall) return a structured `requires_build_step: true` envelope rather than misleading "broken" output — the Tier 3 sidecar that handles them is downstream. CSP + iframe sandbox is the trust boundary at the *content* level, mirroring §1.16.0's Tier-0 Worker boundary at the *execution* level. Sized as a single feature minor (3 tools + 1 Service Worker + 1 host page + settings row + tests). Tiers 2 (console + error capture, the one that catches the Sokoban class specifically) and 3 (driveable preview — Playwright sidecar + selector-shaped tools, plus the `preview_eval` per-invocation-gate question) phase behind dogfood signal per the §"Measurement before scale" commitment — each its own minor, gated on a measured class of bug Tier 1 cannot serve. Full design (per-tier tool surface mirroring the Claude Code preview MCP, security boundary breakdown, failure modes, comparison to existing in-loop tools `submit_script_for_approval` / `submit_plan_for_approval` / `ask_user`) at [`docs/DESIGN-preview.md`](DESIGN-preview.md). **Supersedes** the "Browser-in-browser preview" stub in §"3.0 / Post-2.0 candidates" — pulled forward on the strength of the Sokoban evidence. **Independent of the Profiles arc** and independent of Sandbox / Plugin Discoverability / Retrieval ingest hardening.
...
| **2.1.0** | Phase 2 | `chat_multi.v1`, `rp.v1`, `kb.v1` profiles registered (`DESIGN-profiles.md` line 455). Inheritance through one level (base → leaf); per-profile worked-example test fixtures. Surface count goes from 1 to 4. |
| **2.2.0** | Phase 3 | Operational maturity — task boundary detection heuristics (replaces explicit markers), novelty-score tuning from real usage, per-profile dashboards. |
| **2.3.0+** | Phase 4 | Extensibility — custom profile authoring API, profile diffing, profile regression testing harness. Unblocks plugin-defined surfaces. |
