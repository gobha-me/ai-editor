...
> Last updated: 2026-05-14 (2.49.0.0 in-track — slice 1 of github#24 Phase 1 sub-agents: `subagent.v1` profile + `resolveSubAgentConfig` + `ToolRegistry.executeWithProfile` + `applySubAgentToolFilter` + `State.subagents` slot + `pendingSubAgentApproval` helpers + `'delegate_task'` joins `USER_PAUSE_TOOLS`. Zero user-observable surface; tag does not fire until slice 2 ships the approval card + sub-agent loop). Current released: **v2.48.0.1** (sub-patch under the 2.48.0 tag) · `main` HEAD: 2.49.0.0 (sub-patch space; pre-tag). Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.49.0 — slice 2 (the `delegate_task` tool + approval card pair + transcript panel + `coder.v1` admission + `SUBAGENT_INSTRUCTIONS` prompt block + cost-attribution threading + sub-agent runner glue + persistence + Settings UI row + browser test; tag `v2.49.0` fires here).
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.49.0 — Phase 1 sub-agents (`delegate_task` tool, github#24).** Design-locked at [`docs/DESIGN-sub-agents.md`](DESIGN-sub-agents.md); 2.48.0 tool-loop core extraction (Phase 0) cleared the prerequisite refactor. Sliced into **two PRs** in `X.Y.Z.N` space per [`docs/VERSIONING.md`](VERSIONING.md) — see CHANGELOG `[2.49.0.0]` for the slicing rationale (single-PR precedent vs. half-built admission state). **Slice 2** (next): the `delegate_task` tool + `SubAgentApprovalCard` pair + `SubAgentTranscriptPanel` + `coder.v1.tools.static` admission + `SUBAGENT_INSTRUCTIONS` prompt block + cost-attribution threading (`LLM.chat` `costAttribution` arg) + sub-agent runner glue (uses `tool-loop-core.runToolLoop`) + persistence (`conv-{id}.subagentTranscripts`) + Settings UI row + browser test. End-to-end; tag `v2.49.0` fires here. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. Next `RE-EVAL following 2.49.0` slot fires after 2.49.0 ships (target #4: `git-providers/base.js` 43-method base interface). | `[medium]` |
| ~~**Just shipped (2.49.0.0)**~~ | ~~**Slice 1 of github#24 Phase 1 sub-agents.**~~ ✅ shipped 2026-05-14. Pure plumbing; zero user-observable surface. `subagent.v1` profile in `SYNTHETIC_ENTRIES`; `resolveSubAgentConfig`; `ToolRegistry.executeWithProfile` + `checkRoleAccessForProfile` (additive entry-points; existing `execute` / `checkRoleAccess` delegate); `applySubAgentToolFilter` in the `getToolsForRole` composition chain (no-op against unregistered `delegate_task`); `State.subagents = {tree, transcripts, session_cost}` slot; `pendingSubAgentApproval` + helpers in `js/chat/state.js` (mirror `pendingScriptApproval`); `cancelToolLoop` releases an in-flight sub-agent approval. `'delegate_task'` joins `USER_PAUSE_TOOLS`. 23 new subtests across 3 test files + 2 pinning tests absorbed the new entries. Full Node suite 3326 pass / 1 skipped / 0 fail. | `[strong] [S]` (shipped) |
...
**Next re-eval slot: `RE-EVAL following 2.49.0`** — anchored per the 3-code-minor cadence (3 code minors past 2.46.0 = 2.49.0). ICD-backfill target #4 (`git-providers/base.js` 43-method base interface — contract already exists in code; needs writing up to be cite-able) is the code-aware target.
...
4. **`git-providers/base.js` 43-method base interface** — contract already exists in code; just needs writing up to be cite-able. Target: `RE-EVAL following 2.49.0`.
...
| Touch 3 Window v2 / Sessions | None — `docs/DESIGN-sessions.md` will need authoring | `[medium]` retained at `RE-EVAL following 2.46.0` — 2.0.0 dependency cleared (the Touch 3 design's hard prerequisite); 6 architecture questions parked in [`discussion/touch-3-window-v2-sessions.md`](discussion/touch-3-window-v2-sessions.md). Promotion to `[strong]` blocked on DESIGN-sessions.md authoring slot. |
| MCP discovery Phase 2 OAuth flows ([github#27](https://github.com/gobha-me/ai-editor/issues/27)) | None — `docs/DESIGN-mcp-oauth.md` will need authoring | `[medium]` retained at `RE-EVAL following 2.46.0` — security surface warrants paper-first contract; one external request is enough pull given the OAuth contract implications. DESIGN-mcp-oauth.md authoring slot queued (no specific milestone target yet). |

Translation post-2.47.0.1: the **audit-sweep arc closed** at 2.44.0 (22 entries resolved across 11 system buckets between 2.33.0 and 2.44.0); the **second re-eval** ran at 2.45.0 and surfaced one `[strong]` row for 2.46.0 (shipped); the **third re-eval** ran 2026-05-14 (doc-only, accumulated in `[Unreleased]`, no version bump) and surfaced one `[strong]` row originally targeting 2.47.0. **A user-surfaced diagnostic dump intervened**: two runtime stale-reference regressions (`window.startWorkOnIssueFromList` + `Providers.enrichModels` facade gap) took precedence and shipped as 2.47.0, with the pre-approved docstring cleanup re-scoped to **2.47.0.1 (shipped 2026-05-14 — bundled retrieval-module + registry-module docstring sweep; absorbed the third RE-EVAL `[Unreleased]` content into its versioned heading)**. The two named near-term features (Window v2 / Sessions, MCP OAuth) stay `[medium]` with DESIGN-doc authoring as their promotion preconditions. No `[strong]`-band rows are queued for 2.48.0.
