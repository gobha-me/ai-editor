...
> Last updated: 2026-05-07 · Current released (tagged) version: **1.9.1** · `main` HEAD: **1.9.1**. All of `1.6.6`–`1.6.14`, `1.7.0`–`1.7.2`, and `1.8.0`–`1.9.1` are now tagged on the release line (the "untagged in main" backlog from earlier roadmap revisions has cleared). The 1.8.3 → 1.9.1 stretch closed AST Phase 2 (lever B shipped 1.8.1) and shipped structured user-interaction in two halves: 1.9.0 added the `ask_user` tool, 1.9.1 added the queued-user-input drain — both halves of [github#33](https://github.com/gobha-me/ai-editor/issues/33), now functionally complete. 1.8.5 sits between them as a cost-reporting accuracy fix (`extractUsage()` provider parsing).
...
| **Later** | Open issues #37 (Phase 2), #27, #25, #24, #18 (see *Known open issues* below) — none currently on the active track. |
...
|---|---|---|---|
| **A. Branch switcher upgrade** | Turn the existing static branch view at the top of the left rail into a real switcher: ahead/behind counts, switch / delete / cut-release inline | `left-pane-v2.jsx` "branches" view | Self-contained on existing branch UI; no rail-rework dependency. ~1 patch. |
| **B. ▶ Start prominence on issues** | Elevate the existing auto-branch-on-session-start flow — make the entry button prominent in the issue list row | `left-pane-v2.jsx` "issues" view | The flow already exists (auto-branch creation on session start, with the multi-start guard). This is UX surfacing, not new logic. ~1 small patch. |
...
- **github#25 — `Plan Mode — read-only planning phase with approval gate`** — In-app analogue of Claude Code's plan mode. Adjacent to the `ask_user` interaction track that just shipped (1.9.0–1.9.1); naturally chains off the same `pendingUserResponse` seam. Self-contained; medium effort.
