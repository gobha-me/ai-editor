...
> Last updated: 2026-05-10 (post-2.10.0) · Current released: v2.10.0 · `main` HEAD: 2.10.0.
...
| Phase | Track |
|---|---|
| **Just shipped** | Shipped through v2.10.0 — see [CHANGELOG.md](../CHANGELOG.md). Recent: 2.8.0 (`kb.v1` picker promotion + per-chat profile binding); 2.9.0 (provider rate-limit pacer — Touch 3 Window v2 / Sessions hard prerequisite cleared); 2.9.1 (anti-loop refusal payload — concrete recovery candidates at streak ≥ 3); 2.10.0 (in-editor preview **Tier 3a** — selector-shaped driveable tools: `preview_snapshot` / `_click` / `_fill` / `_inspect` / `_resize`; `preview_eval` decision settled — does not ship). |
| **Now** | Between active tracks post-2.10.0. The Phase 2 picker-promotion track closed for ai-editor with `kb.v1` (2.8.0); `chat_multi.v1` / `rp.v1` are deprioritized for this product (no real consumer — see *Next* row). The next slot is open; candidates are listed in *Next* / *Later* below. Per the doc's own discipline (§"How to read this doc"), shipped detail lives in [CHANGELOG.md](../CHANGELOG.md), not here. |
