...
> Last updated: 2026-05-14 (`RE-EVAL following 2.49.0` running — fourth re-eval slot under the methodology adopted 2026-05-12; doc-only, no version bump consumed; deliverables accumulate in `[Unreleased]`. Output: ICD-git-providers.md authored (target #4: `BASE_GIT_PROVIDER` 55-method surface + 4 concrete providers + circuit breaker) + paper-half drift fixes + ROADMAP "Now"/"Just shipped" row roll-forward. Three code-aware findings queued: `getCommitDiff` base-promotion, GitLab partial-capabilities shape test, `resolveReviewThread` dead-code decision). Current released: **v2.49.0** (Phase 1 sub-agents end-to-end via `delegate_task`; tag fired 2026-05-14). Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: one of the three queued code-aware findings from the ICD #4 session promotes to `[strong]` in the next code minor (likely 2.50.0); choice deferred to the row scheduling pass.
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **`RE-EVAL following 2.49.0`** (running 2026-05-14; doc-only, no version bump consumed — deliverables accumulate in `[Unreleased]` per the policy refinement at the third re-eval slot). Code-aware half: **ICD #4 — `BASE_GIT_PROVIDER` 55-method surface + 4 concrete providers + circuit breaker** authored at [`docs/ICD-git-providers.md`](ICD-git-providers.md). Paper-half: this "Now" row + the line-3 header roll-forward + the §"Per-subsystem ICD backfill program" target-#4 strikethrough + the ARCHITECTURE.md 43-method → 55-method drift fix + the DESIGN-git-providers-and-ui-extensions.md §2 forward-pointer to the ICD. **Three** code-aware findings queued for the next code minor's `[strong]` slot: (a) `getCommitDiff` informal-extension (three of four providers implement it; missing from base — single-file `[strong] [S]` candidate); (b) GitLab partial-capabilities shape test (anti-regression for the `undefined → false` invariant); (c) `resolveReviewThread` dead-code decision (promote vs demote). Selection deferred to the row scheduling pass. After this slot: Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. Next re-eval slot is `RE-EVAL following 2.52.0` (3 code minors past 2.49.0). | `[rewrite session]` (re-eval) |
...
Translation post-`RE-EVAL following 2.49.0`: the **audit-sweep arc closed** at 2.44.0 (22 entries across 11 buckets, 2.33.0 → 2.44.0). Three re-evals ran 2026-05-14: **second** at 2.45.0 (surfaced one `[strong]` row, shipped 2.46.0); **third** as `RE-EVAL following 2.46.0` (doc-only; surfaced one `[strong]` row originally targeting 2.47.0, re-scoped to 2.47.0.1 after a user-surfaced diagnostic dump intervened with the two runtime stale-reference regressions that shipped as 2.47.0); **fourth** as `RE-EVAL following 2.49.0` (doc-only; ICD #4 git-providers authored; surfaced **three** `[strong]`-band candidates queued for the next code minor — `getCommitDiff` base-promotion, GitLab partial-capabilities shape test, `resolveReviewThread` dead-code decision). The 2.48.0 → 2.49.0 minor arc shipped tool-loop core extraction (Phase 0) + sub-agents Phase 1 end-to-end (slice 1 in 2.49.0.0 + slice 2 in 2.49.0). The two named near-term features (Window v2 / Sessions, MCP OAuth) stay `[medium]` with DESIGN-doc authoring as their promotion preconditions. No `[strong]`-band rows are queued for 2.50.0 yet — selection of which of the three ICD #4 findings promotes is deferred to the row scheduling pass.
