...
> Last updated: 2026-05-14 (2.48.0.1 shipped — in-track patch under the 2.48.0 tag: gitea#415 file-edit tool surface fixes; bundled `edit_file` `edits`-key shape detection + description rewrite + `validateToolParameters` truncation-blame removal). Current released: **v2.48.0.1** (sub-patch under the 2.48.0 tag) · `main` HEAD: 2.48.0.1 + fresh `[Unreleased]`. Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.49.0 — Phase 1 sub-agents (`delegate_task` tool, github#24); design-locked at [`docs/DESIGN-sub-agents.md`](DESIGN-sub-agents.md), unblocked by the 2.48.0 tool-loop core extraction.
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.49.0 — Phase 1 sub-agents (`delegate_task` tool, github#24).** Design-locked at [`docs/DESIGN-sub-agents.md`](DESIGN-sub-agents.md); 2.48.0 tool-loop core extraction (Phase 0) cleared the prerequisite refactor. Scope per the DESIGN doc: `delegate_task` tool, `subagent.v1` profile, approval card, transcript panel, cost attribution plumbing. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. Next `RE-EVAL following 2.49.0` slot fires after 2.49.0 ships (target #4: `git-providers/base.js` 43-method base interface). | `[medium]` |
