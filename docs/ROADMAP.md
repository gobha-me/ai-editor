...
> Last updated: 2026-05-01 · Current released version: **1.3.9** · Authoring branch: `feat/roadmap-1.x`
...
> **Renumbering note (2026-04-30 kickoff, revised post-1.3.2 ship).** The original §1.3.1 ("workspace scope") shipped inside 1.3.0; the original 1.3.1 self-healing-tools work also already shipped inside 1.3.0 (`memory_revise` with required `reason` parameter, audit-entry-per-revision in [js/intelligence/memory/store.js](js/intelligence/memory/store.js)). **The "Reasoning as turn metadata" item — originally numbered 1.3.6, then 1.3.5 — shipped as 1.3.1 (2026-04-30)**, taking the slot the reframed self-healing-tools item vacated. **Cross-device session sync (originally §1.3.3) shipped as 1.3.2 (2026-04-30)** — versions are monotonic releases, not reserved slots, and the previously-§1.3.2 persona-scope item was deferred indefinitely with no version reservation; the next slot opened for whatever shipped next, which was session sync. Session replay slides up to §1.3.3. Subsequent items keep their numbers; new memory follow-ups slot above 1.3.3 as they're scoped.
>
> **Design refined at session-sync kickoff: per-conversation opt-in, not workspace-wide.** Memory's repo mode is workspace-level because memory holds curated facts (intentional, pre-vetted). Sessions hold raw transcripts (pasted secrets, stack traces, half-formed scratch thinking the user never planned to commit), so workspace-level opt-in was too coarse. The shipped gate is a per-conversation `synced: boolean` flag on the index entry; default-false; the conversation drawer carries the toggle. Workspace-wide opt-in remains the memory pattern.
>
> **Renumbering note 2 (2026-04-30, post-1.3.4 ship — Touch 2 facelift slots in).** Touch 2 design deliverable from claude.ai/design landed earlier on 2026-04-30 (commit `69f6597`, stored at `docs/design/touch-2-facelift/`) — **earlier than Decision §10 had projected** ("between 1.4.x winding down and 2.0.0"). Decision §10's expectation was wrong; the facelift work begins now. The user's call (made at 1.3.5 kickoff): facelift patches first, 1.4.0 (Tools admission) waits until they wind down. The Touch 2 deliverable's pushback memo locks **top bar Restructure** and **Settings sidebar Restructure** for ship; net-new surfaces (Connections, Debug, Help) have full artboards; chat panel stays in exploration. The token vocabulary `--tk-*` is the public contract every shipped facelift component reads from. The 1.3.x track now extends through the facelift — see the projected slots below. **No memory follow-ups currently in flight; the slots below are facelift work.**
...
- **1.3.2 [SHIPPED — 2026-04-30]:** Cross-device session sync via Git. Each conversation in the drawer carries a `synced: boolean` flag (default-false); flipping it on writes `.aieditor/sessions/<id>.json` to the pending buffer and pending-stages it on the next commit, mirroring the Flow 3A/3B commit-modal pattern from memory. Conflict resolution: latest `updatedAt` wins. JSON schema (locked at PR kickoff): `{ schema_version: 1, id, title, createdAt, updatedAt, messageCount, messages, summaryInfo, pruneStash, synced_by, last_synced_at }`. Drawer gains 📡 sync badge + per-conversation toggle (UI #6). First-toggle-on confirm dialog explains the privacy implication; untoggle confirm explains that the already-committed file persists. Out of scope: auto-deletion on untoggle (manual `git rm`), divergent-message-list conflict UI, encrypted storage. See [CHANGELOG.md §1.3.2](../CHANGELOG.md) for full detail. **Persona scope** (the prior §1.3.2 occupant) remains [DEFERRED INDEFINITELY] — single-user code-focused editor + 2.0 ships `chat_multi.v1` / `rp.v1` profile stubs already, so persona isn't load-bearing for the planned profile contract; revisit only if `user`-scope memory usage in production reveals cluster patterns that would meaningfully reduce noise.
- **1.3.3 [SHIPPED — 2026-04-30]:** Session replay / shareable transcripts. Conversations export as a single `.aieditor.session` JSON file (the same `serialize/parse` shape and `schema_version: 1` 1.3.2 locked); drop one into any AI Editor instance and a read-only stepper walks every message, tool call/result, reasoning bubble, and edit-tool context window. Read-only by construction — replay state lives in `js/chat/replay.js` closures and never enters `ConversationManager`, so loading an archive can't collide with a same-id local conversation. Per-row ⤓ download in the conversation drawer; ▶ button in the chat header opens the modal in empty drop-zone state. No new schema fields, no `schema_version` bump — replay is the second consumer of the 1.3.2 contract, validating that the schema isn't sync-only. Out of scope: full-file before/after snapshots in edit-tool results (3-line `context` window stays the surfaced view; separate admissibility argument), forking from a turn into a live conversation, and side-by-side comparison. See [CHANGELOG.md §1.3.3](../CHANGELOG.md) for full detail.
- **1.3.4 [SHIPPED — 2026-04-30]:** Tools-track foundation (PR 1 of §1.4.0 Phase 1). Lands `js/intelligence/tools/` with the `ToolDef`/`ToolID`/`Catalog` data contracts and a read-only adapter over `js/tools/registry.js` — no admission decisions, no model-visible change. `coder.v1.tools.static` populated with the §1.4.0 always-loaded set; meta-tool names included even though the implementations don't exist yet (the consumer in PR 2 skip-not-throws on unresolved names). Same scaffolding pattern that landed `js/profiles/` data in 1.1.0 ahead of subsystem wiring; defers the 1.4.0 minor bump until Phase 1's exit criteria (70%+ token reduction, working discovery roundtrip, debug-modal diagnostics) are satisfied. Not a memory follow-up — first item under §1.3.x that crosses tracks, sequenced here because 1.3.0–1.3.3 closed out memory's load-bearing work and `[Unreleased]` was empty. See [CHANGELOG.md §1.3.4](../CHANGELOG.md) for full detail.
- **1.3.5 [SHIPPED — 2026-04-30]:** Theme tokens contract foundation (PR 1 of the Touch 2 facelift arc). Lands `css/themes/{tokens, refined, editorial}.css` with the frozen `--tk-*` public CSS-custom-property vocabulary; aliases the legacy `--bg-primary`/`--text-primary`/`--accent`/etc. in `css/base.css :root` to `--tk-*` so component CSS reads through the contract with zero edits; sweeps standalone hex literals from component CSS into `--tk-*` references; ships **Refined IDE** (default; mirrors today's palette one-for-one — zero visual regression) and **Editorial Calm** (bundled alternate; serif headings, warm neutrals, generous spacing) to prove the contract carries variation; adds Settings → Appearance Theme dropdown; adds CI lint that fails the build on standalone hex outside `css/themes/`. Pushback PROBE-driven: *"a frozen token vocabulary; once published, removing a token is a breaking change."* Foundation-first because every later facelift component must read its colors/fonts/radii/shadows from `--tk-*` to be theme-able — shipping a component before the contract is frozen would force redo or quietly couple to specific theme values. See [CHANGELOG.md §1.3.5](../CHANGELOG.md) for full detail.

- **1.3.6 [SHIPPED — 2026-04-30]:** Top bar Restructure (Touch 2 LOCKED) — PR 2 of the facelift arc. Rebuilds `html/header.html` to the LOCKED-for-ship Restructure layout per the design pushback memo (`docs/design/touch-2-facelift/project/pushback.jsx` line 29): brand mark + divider + branch indicator + ⌘K command surface + token-usage pill + 3-icon row (Settings / Help / Debug). New `css/topbar.css` + `--tk-header-height` / `--tk-cmd-placeholder` tokens; every value reads through the 1.3.5 `--tk-*` contract — no hex literals, no per-theme variants of the component itself. Both shipped themes (Refined IDE, Editorial Calm) render the same markup. Revert relocates to the editor toolbar (`html/editor-panel.html`); plugins relocate to Settings → Plugins → Toolbar actions; Error log + LLM debug log consolidate into a Debug dropdown bridge until §1.3.9 ships the slide-out; the cost-reset ✕ button drops from the pill (still callable as `window.resetSessionCost()` until §1.3.9). **⌘K Phase 1** = file-search shell that aliases the existing Ctrl+P QuickOpen; commands and settings/help search accrete in 1.3.7+. Branch indicator renders branch name only in 1.3.6 — **ahead/behind counts deferred to 1.3.6.1** (net-new git-provider work across GitHub / Gitea / GitLab / local-zip; the `#tbBranchCounts` element exists hidden, ready to populate). Model picker → chat compose stays out of this patch (design artboard for destination layout incomplete in `topbar.jsx`). See [CHANGELOG.md §1.3.6](../CHANGELOG.md) for full detail.
- **1.3.7 [SHIPPED — 2026-05-01]:** Settings sidebar Restructure (Touch 2 LOCKED) — PR 3 of the facelift arc. Replaces the 13-tab horizontal strip in `html/modals.html` with a vertical 196px sidebar grouped **Workspace** (Connections, Ignore) / **AI** (LLM, Models, Context, Embeddings, Roles, Memory) / **App** (Appearance, Plugins, Storage, Cost, Advanced) per `docs/design/touch-2-facelift/project/settings.jsx`. New `css/settings-sidebar.css` reads every value through the 1.3.5 `--tk-*` contract — no hex literals, no per-theme variants; Refined IDE and Editorial Calm render identical markup. Modal header gains a **local search input** (`#settingsSidebarSearch`) that filters sidebar items by label as the user types and collapses groups whose items are all hidden — independent of the global ⌘K palette so it works the moment the modal opens. Esc clears; modal-open auto-focuses the input. Sidebar items keep the `.settings-tab` class so the existing tab-switching loop in `js/settings-manager.js` matches them by selector with no JS-contract break; horizontal-strip scroll helpers (`initTabArrows`, `window.scrollSettingsTabs`) drop. Help modal still uses the shared `.settings-tabs-*` strip CSS in `css/modals.css` until §1.3.10 retires it. **Lucide icons deferred to §1.3.11**; this patch is structurally focused. See [CHANGELOG.md §1.3.7](../CHANGELOG.md) for full detail.
- **1.3.8 [SHIPPED — 2026-05-01]:** Connections panel (Touch 2 net-new surface) — PR 4 of the facelift arc and the first net-new surface in it (1.3.5–1.3.7 were foundation + relocations). Replaces the flat `connection-card` list + single Add button with the LOCKED Restructure shape per `docs/design/touch-2-facelift/project/connections.jsx`: provider-grouped layout (`.conn__group` per registered provider), per-provider "Add ${provider} account" affordance, per-row status pill (ok / warn / disabled). Status resolves from data already on the connection (`enabled`, `_unreachable` from the registry circuit breaker, `token` presence) — no new persisted fields. The data model (`State.settings.connections[]`) was already array-shaped and `GitProviderRegistry.listAllRepos()` already aggregated across enabled connections; the UI was what wasn't keeping up. New `css/connections.css` reads every value through the 1.3.5 `--tk-*` contract — no hex literals, no per-theme variants; both shipped themes (Refined IDE, Editorial Calm) render identical markup. **Aggregated repo picker deferred to 1.3.8.1** (the `variant === "with-picker"` block lives outside the Settings modal — project loader / sidebar dropdown; bundling it would double the PR; pairs naturally with `lastSyncAt` plumbing because the picker is what actually drives `listAllRepos`). **Bitbucket out of scope** — no provider client exists; the new layout is data-driven over `GitProviderRegistry.list()`, so adding Bitbucket later is registering a provider, not redesigning the panel. See [CHANGELOG.md §1.3.8](../CHANGELOG.md) for full detail.
- **1.3.9 [SHIPPED — 2026-05-01]:** Debug slide-out (Touch 2 net-new surface) — PR 5 of the facelift arc and the second net-new surface in it. Replaces the 1.3.6 dropdown bridge (`#tbDebugDropdown` with "Error log" + "LLM debug log" menu items) and retires the standalone `#errorLogModal` / `#llmDebugModal` in favor of a single right-edge drawer per `docs/design/touch-2-facelift/project/debug.jsx`. Five tabs — Logs (level chip filter, subscribes to a new `error:logged` event), Connections (provider-grouped git rows resolved through the same `statusFor` the Settings → Connections panel uses, plus a single AI-provider summary), Indexer (queue / paused / indexed counts off `ContextManager` plus a "Re-index from scratch" action), AI (`LLMDebug.exchanges[]` table with click-to-expand per-exchange detail via the new exported `renderExchangeDetail`, factored out of `renderLLMDebug`), Plugins (`Plugins.list()` plus an in-memory error buffer fed by a new `plugin:initError` event from `Plugins.init` and `plugin-loader`'s install/load paths). Head row carries Pause stream (suppresses live re-renders; underlying capture continues so the bug-report flow still works after unpause), **Copy bundle** (one-click clipboard JSON: errors + exchanges + connections + indexer + plugins + version + ts), and Esc-closeable Close. **First slide-out shell in the codebase** — new `css/slide-out.css` (`.slide-out-overlay` + `.slide-out`) is the contract that 1.3.10 (Help) and any later right-edge drawer inherits; reads every value through the 1.3.5 `--tk-*` contract, no hex literals, no per-theme variants; both shipped themes render identical markup. **AI-provider circuit breaker deferred** (parallel to git `_unreachable`, its own data-layer change). **Plugin runtime error capture beyond init deferred** — the patch covers install / load / init throws. See [CHANGELOG.md §1.3.9](../CHANGELOG.md) for full detail.
- **1.3.10: Help slide-out (Touch 2 net-new surface).** New right-edge slide-out per `docs/design/touch-2-facelift/project/help.jsx`: left-rail nav grouped Getting Started / Building / Concepts / Reference; data-driven hotkeys page that renders ⌘ on macOS and Ctrl on Win/Linux from the same data source; Plugin SDK page; search-all across docs. Replaces today's single icon that opens an external marketing page. Wires the in-app help renderer that PLAN.md / UI #14 has flagged for two minor versions.
- **1.3.11: Lucide icon family swap (Touch 2 PROBE).** Replace emoji in UI surfaces (⚡ in brand mark, 👋 in chat welcome, 🚀 in zip upload, file-tree action emoji, etc.) with Lucide icons (1.6 stroke weight, line style, ~1500 icons, MIT, same family as Vercel / Linear / Replicate). Emoji stays for user content (chat messages, commit messages, code comments); UI uses Lucide only. Self-contained — touches CSS for sizing + a vendor bundle entry; no behavioral change.
- **1.3.12: Self-hosted woff2 fonts (Touch 2 PROBE).** Replace the system font stack with woff2-self-hosted Inter (UI under Refined IDE), IBM Plex Sans + Source Serif 4 (UI under Editorial Calm), and JetBrains Mono / IBM Plex Mono (code surfaces). The 1.3.5 theme files declare the stack with system fallbacks; this patch self-hosts via `assets/fonts/` so behavior is consistent across OSes. Pre-1.3.12: the editor looks subtly different on macOS vs Windows vs Linux because the system stack falls through differently; post-1.3.12: every user sees the same hierarchy and metric.
- **1.3.13: rem-based UI scaling (replaces 3-axis font-size sliders).** Today Settings → Appearance has three independent font-size sliders (UI / chat / editor) backed by `--ui-font-size` / `--chat-font-size` / `--editor-font-size` in `css/base.css`. Replace with a single **UI scale** slider 80%–175% (default 100%) that scales the rem base — every surface that derives sizing from `--ui-font-size` and the chat-derived `--chat-*` calc chain shifts proportionally. Keeps editor font size as a **separate** slider (code is the one place users genuinely want a different size from the rest of the UI). Per Jeff's request at 1.3.5 kickoff: simpler, more predictable, plays better with browser zoom and accessibility settings. Migration: legacy `fontSize`/`chatFontSize` get folded into the scale slider via the larger of the two; `editorFontSize` keeps its own knob. **Why 1.3.13 and not 1.3.5:** the slider rebuild interacts with the new Settings sidebar (1.3.7) — landing it before sidebar Restructure means re-laying-out the slider twice. Wait for the sidebar shape to settle.
...
- **1.3.5: Reasoning as turn metadata [SHIPPED AS 1.3.1 — 2026-04-30].** *Slot retained below for the design rationale, which still describes the shipped implementation. The work landed in 1.3.1 because §1.3.1's prior occupant (self-healing tools) already shipped inside 1.3.0; rather than leave a numbered gap, the highest-value backlog item slid into the slot. See [CHANGELOG.md §1.3.1](../CHANGELOG.md) for the as-shipped detail; section below remains the canonical design record.*
  ...
  **Why now:** today the streaming layer treats reasoning as text-to-strip rather than data-to-preserve. Two visible failure modes flow from that — (a) when a closing `</thinking>` tag splits across SSE chunks, fragments leak into the rendered response (the duplicated-preamble symptom users see in chat); (b) reasoning content is destroyed during stripping, so we can't show it on demand, can't include it in session exports (1.3.4), can't count its tokens separately (1.2.x cost dashboard), and can't surface it for debugging when the model produces a wrong answer. Splitting reasoning into its own field is the same fix as the bug — once admitted explicitly, the leak path doesn't exist by construction. **Architectural principle: admissibility, not accumulation.** Reasoning currently accumulates into `content` by leakage and is partially stripped after; the fix is to admit it as a first-class turn property with its own field, its own UI treatment, and its own cost accounting.

  **Why this slot, not 1.1.x:** the original framing of this work was "land alongside 1.1.0 turn-metadata enrichment because both touch the same structures." The 1.1.x track has shipped; the schema landed in 1.1.0 is read-path-only and intentionally open to additive fields without migration, so 1.3.5 plugs into the same contract retroactively. 1.3.4's session-replay export is the immediate consumer; sequencing right after it keeps the field's first non-trivial reader in the same minor track.

  **Touchpoints:**
  - `js/llm/api.js:700-726` — streaming-loop `<think>`/`<thinking>` detection. Today the `thinkBuffer` content inside a block is discarded (`continue` at line 704); the patch routes it into a new `reasoning` accumulator alongside `content`, returned to the caller as `{content, reasoning}`. The split-across-chunks edge case is what makes today's stripping leaky; explicit accumulation closes it.
  - `js/llm/utils.js` — `stripThinkBlocks` becomes a fallback used only when reasoning capture is unavailable (older provider responses, fixtures). Renamed to `splitThinkBlocks(text) → {content, reasoning}` with the legacy stripping name kept as a thin wrapper for plugin compatibility.
  - `js/chat/state.js` — assistant turn shape grows `reasoning?: ReasoningBlock | null` (see open question on shape).
  - `js/chat/handlers.js` — replaces the four stripping calls (lines 453, 749, etc.) with the split path; routes the captured reasoning into `addMessage(..., { reasoning })`.
  - `js/chat/messages.js` — replaces the three render-time `stripThinkBlocks` calls (lines 155, 184, 298) with rendering `reasoning` as a collapsed inline bubble above the response. Match the existing themed dialog/inline-card system; one paragraph of UX spec is enough at this stage.
  - `js/chat/export.js` — already has a `reasoningTokens` slot in the cost summary (line 111); extend the export shape to include the captured reasoning text per turn so 1.3.4's replay viewer can step through what the model thought at each step.
  - `js/intelligence/cost/cost-recorder.js` — reasoning tokens count toward the **next** turn's input cost, not the emitting turn's output cost (provider-billed-as-input). Reasoning attribution is already a column in the cost dashboard's per-conversation breakdown today; this patch ensures the column actually populates from captured-reasoning length rather than from a zeroed slot.
  - `js/chat/turn-enrich.js` — currently enriches tool-result turns only; assistant-turn enrichment is added by this patch (small surface — just merges `reasoning` onto the assistant skeleton).
  - `js/intelligence/compression/` — no rule changes. Compression treats reasoning as part of its turn (intentional: reasoning without its turn is meaningless; a turn without its reasoning loses provenance). Document this in the rule preamble; no code change.
  - Test fixtures: any `.mjs` suite that asserts on stripped chat content needs the response shape updated so split-out reasoning is asserted on a separate field.

  **Open questions to resolve before implementation:**
  1. **`reasoning` field shape.** Different providers emit reasoning differently: Claude `<thinking>` / `<think>` tags, Venice's format, OpenAI o1-style hidden reasoning chunks, Ollama-hosted reasoning models. **Lean structured:** `{ provider, format: 'tag'|'native'|'channel', content, started_at, ended_at } | null` — we'll regret a string when the third format arrives. Plain string is simpler today but forecloses the cost-attribution and per-format rendering decisions a year from now. Resolve at PR kickoff.
  2. **Backwards compatibility.** Old turns in stored conversations have no `reasoning` field. The renderer must treat absence as "no reasoning to show" — *not* as "reasoning is empty string" (an empty string would still render the bubble shell). Renderer guard: `reasoning && reasoning.content && reasoning.content.length > 0`.
  3. **Cost attribution.** Reasoning tokens count toward the *next* turn's input cost (provider-billed-as-input), not the emitting turn's output cost. The `reasoningTokens` slot already in `js/chat/export.js` line 111 reflects emit-side; verify the cost dashboard's input-cost calc routes captured reasoning to the *receiving* turn so per-turn totals don't double-count.

  **Exit criteria:**
  - A reasoning model session produces history turns with non-null `reasoning` on the assistant side; older turns persist with `reasoning: undefined` and the renderer collapses to no-bubble.
  - The "duplicated preamble" symptom (closing-tag-split-across-SSE-chunks) does not reproduce on a 50-turn reasoning session (regression test: an `.mjs` fixture that drips a `</thinking>` tag across two chunks).
  - 1.3.4 session-replay export round-trips captured reasoning unchanged (asserts on the export-shape `.mjs` test).
  - The cost dashboard's per-conversation `reasoningTokens` column is non-zero for a session that exercised a reasoning model.
  - **Removability check:** with `reasoning` ripped out (renderer falls back to `stripThinkBlocks`), what user-visible behavior degrades? Reasoning bubbles disappear; replay export loses one column; cost dashboard shows zero reasoning-token cost. The duplicated-preamble bug returns. If "nothing measurable" — the field wasn't earning its complexity and 1.3.4 didn't get the consumer it needed.

  **Size:** Single PR, ~600-800 lines (streaming change + state.js + render + export + a regression-fixture test). Constraint: don't redesign the turn schema beyond adding the one field; the 1.1.0 schema is already in flight (now shipped) and this is an additive read-path-only patch, not a revision. Constraint: don't try to solve the duplicated-preamble bug as a separate fix — once reasoning is split instead of stripped, that bug closes by construction; a "preamble dedup" patch would be a workaround, not a fix.

  **UI impact:** Collapsed-by-default reasoning bubble above each assistant response that has reasoning attached. Click to expand; matches the existing inline-card theming (no new design system; no new CSS tokens beyond what `mem-consent`/`tool-call-details` already established). UI #23 in the cross-cutting table below.
...
10. **claude.ai/design engages on a two-touch model.** Touch 1 (pre-1.3.0): Memory UX scoped to the three flows in §1.3.0 (consent prompts, Memory tab, commit-modal warning). Kickoff prompt handed off 2026-04-29; **deliverable received the same day and stored at `docs/design/touch-1-memory-ux/`** — read it (and the chat transcript at `docs/design/touch-1-memory-ux/chats/chat1.md`) before scoping the 1.3.0 implementation. Headline outputs: 7-item pushback (3 model-level pushes including "drop persona scope from 1.3.0"; 3 implementation probes; 1 nit), Flow 1 inline consent card with "quiet line" variant, Flow 2 in-strip placement A recommended over sidebar B (B reserved as a 2.0 forcing function), Flow 3 protected-branch escape hatch. The biggest open question raised by the design: drop persona scope (`1.3.2`) from the Memory track to simplify Flow 1 + Flow 2 + the `.aieditor/memory/` file count? **Touch 2 (received 2026-04-30, ahead of plan).** The original projection was *"between 1.4.x winding down and 2.0.0"* — that hypothesis is now broken; the deliverable arrived pre-1.4.0 and is **stored at `docs/design/touch-2-facelift/`** (read `pushback.jsx` + `chats/chat1.md`). Headline outputs: 12-item pushback memo (1 FRAME / 1 GAP / 3 PUSH including 1 LOCKED-for-ship-as-Restructure / 1 DECIDED killing the Reskin variant / 4 PROBE / 2 NIT); top bar Restructure + Settings sidebar Restructure both LOCKED; three net-new surfaces (Connections, Debug, Help) with full artboards; chat panel kept in exploration. The pushback's strongest PROBE — *"a frozen `--tk-*` token vocabulary; once published, removing a token is a breaking change"* — is what 1.3.5 ships. The 1.3.x track now extends through the facelift series (1.3.5 → 1.3.13) before 1.4.0 opens. **Lesson learned:** the "between 1.4.x and 2.0.0" projection was a guess at what made sense given the calendar; the user's actual decision was driven by Touch 2 being too compelling to defer. Don't time-box design availability against subsystem cadence; design lands when the user decides it lands.
