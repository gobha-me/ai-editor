...
> Last updated: 2026-05-14 (third RE-EVAL slot ran — doc-only; **no version bump**, accumulates in `[Unreleased]` per the policy refinement at this re-eval; ICD-backfill target #3 shipped at [`docs/ICD-tool-registry.md`](ICD-tool-registry.md)). Current released: **v2.46.0** · `main` HEAD: 2.46.0 + [Unreleased]. Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.47.0 — first code minor after this re-eval; concrete candidate is the ~12 sibling-file `js/context-manager.js` reference cleanup across `js/intelligence/retrieval/` (deferred from 2.46.0).
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.47.0 — sibling-file `js/context-manager.js` reference cleanup across `js/intelligence/retrieval/`** is the first concrete code candidate post-re-eval. ~12 files surfaced by the 2.46.0 audit pass (`index.js`, `pipeline.js`, `bm25-indexer.js`, `loader.js`, `ledger-consumer.js`, `measurement.js`, `strategies/semantic.js`, `strategies/thematic.js`, `test-corpus.js`, `manager.js:4–5`) — deferred from 2.46.0 to preserve "same module" scoping. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. | `[strong] [S]` |
| **Next (2.48.0+)** | Shape TBD by what 2.47.0 surfaces. Inventory candidates if a new sweep wave opens: dependent on the next audit pass. Touch 3 Window v2 / Sessions and MCP OAuth Phase 2 remain `[medium]`-band candidates awaiting DESIGN-doc authoring. | `[fuzzy]` |
...
| **2.47.0 — sibling-file `context-manager.js` reference cleanup + stale `getToolsForProfile` docstring fix** | [`docs/ICD-tool-registry.md`](ICD-tool-registry.md) §"Code-aware findings" + the 2.46.0 audit pass's out-of-scope finding | `[strong] [S]` for both — bundleable (both are docstring-shape fixes, no behavior change); ~12 retrieval-module files + 1 registry-module file in one PR |
...
Translation post-`RE-EVAL following 2.46.0`: the **audit-sweep arc closed** at 2.44.0 (22 entries resolved across 11 system buckets between 2.33.0 and 2.44.0); the **second re-eval** ran at 2.45.0 and surfaced one `[strong]` row for 2.46.0 (shipped); the **third re-eval** ran 2026-05-14 (doc-only, accumulating in `[Unreleased]`; no version bump) and surfaced one `[strong]` row for 2.47.0. That row bundles cleanly with the 2.46.0 out-of-scope sibling-file finding — both are docstring shape fixes — so 2.47.0 is well-shaped as a single PR. The two named near-term features (Window v2 / Sessions, MCP OAuth) stay `[medium]` with DESIGN-doc authoring as their promotion preconditions.
