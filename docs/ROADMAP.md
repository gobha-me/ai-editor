...
- **1.2.5: Compression settings refresh [+1 week, after 1.2.4 lands].**
  ...
  **What ships:** the Settings → Chat Summarizer panel is renamed to **Settings → Compression** and replaced. Five summarizer-era knobs (`messages_kept_no_tools`, `messages_kept_with_tools`, `summary_trigger_threshold`, `resummarize_interval`, `max_summary_length`) collapse into an architecture-shaped surface that names what the rule pipeline is actually doing:
  ...
  | Eviction rules | Checkbox list: Subsumption, Invalidation, Consumption, Resolution | Default all-on. **Custom-mode-only** (modes 1–3 don't expose the toggles, only present-or-absent). |
  ...
  **Open questions to resolve before implementation:**
  1. **Power-user surface for individual rule toggles.** Current proposal: toggle list visible in Custom mode only. Modes 1–3 hide the checkboxes (the mode names a rule set; toggling them out from under the mode is incoherent). **Push back if too restrictive.** Alternative: always-visible, with the mode radio re-resolving toggle state on selection — but that introduces a "you changed Subsumption to off but kept Balanced — which wins?" UX trap.
  2. **`preserve_recent` per-rule overrides.** Some rules (Resolution) might be safer with a higher floor than others. Current proposal: per-mode default only; revisit if power users complain. Alternative: per-rule overrides under Custom — but four rules × four modes × per-rule override is the surface area we want admissibility against.
...
- **1.3.6: Reasoning as turn metadata [+1 week].**
  ...
  **Touchpoints:**
  ...
  - `js/chat/export.js` — already has a `reasoningTokens` slot in the cost summary (line 111); extend the export shape to include the captured reasoning text per turn so 1.3.5's replay viewer can step through what the model thought at each step.
  ...
  **Exit criteria:**
  ...
  - 1.3.5 session-replay export round-trips captured reasoning unchanged (asserts on the export-shape `.mjs` test).
  - The cost dashboard's per-conversation `reasoningTokens` column is non-zero for a session that exercised a reasoning model.
  - **Removability check:** with `reasoning` ripped out (renderer falls back to `stripThinkBlocks`), what user-visible behavior degrades? Reasoning bubbles disappear; replay export loses one column; cost dashboard shows zero reasoning-token cost. The duplicated-preamble bug returns. If "nothing measurable" — the field wasn't earning its complexity and 1.3.5 didn't get the consumer it needed.
