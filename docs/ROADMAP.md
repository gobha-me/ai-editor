...
> Last updated: 2026-05-02 · Current released version: **1.4.9** · Authoring branch: `claude/jolly-volhard-998f9f`
...
| Phase | Track | Status |
|---|---|---|
| **Now** | Retrieval foundation in-flight (1.4.9 ✓) — Phase 1 (1.5.0) is the next sequenced milestone | Tools track fully shipped through 1.4.8 (Phase 1 at 1.4.0 with 79.5% token reduction live; 1.4.1 semantic `find_tool` ✓, 1.4.2 MCP bridge ✓, 1.4.4 workspace-scoped settings ✓, 1.4.5 test-driven loop ✓, 1.4.6 scan-driven CI logs ✓, 1.4.7 ghost text ✓, 1.4.8 tuning + LRU eviction ✓). 1.4.9 ✓ ships the Retrieval data foundation (ChunkRef contract + ChunkID hash + module scaffolding) ahead of chunkers/strategies that consume it. |
| **Next** | Retrieval Phase 1 (1.5.0) — chunkers, Semantic + Structural strategies, Composer, ledger consumer, migration off `js/context-manager.js` | Designed; foundation laid in 1.4.9; chunker + strategy + Composer PRs sequence next. |
