...
> Last updated: 2026-05-10 (post-2.15.0) · Current released: v2.15.0 · `main` HEAD: 2.15.0.
...
| Phase | Track |
|---|---|
| **Just shipped** | Shipped through v2.15.0 — see [CHANGELOG.md](../CHANGELOG.md). Recent: 2.10.0 (in-editor preview **Tier 3a**); 2.11.0 (Touch 3 **Rail v2** sidebar — Files / Issues / PRs / Branches); 2.12.0 (Touch 3 **PR Review** surface — read-only middle-pane takeover); 2.13.0 (Touch 3 PR Review **slice 2** — review submission + dock + merge folded into the dock); 2.13.1 (Rail badge count-drain after PR merge — fixes a sync-listener vs async-mutation race); 2.13.2 (Touch 3 PR Review **CI polling + Re-run failed jobs**); 2.14.0 (Touch 3 PR Review **slice 5: Diagnose & fix** — failed-CI-log driven single-file patch proposal); 2.15.0 (**MCP discovery Phase 2 slice 1** — Smithery dynamic catalog + search/chip filter + lazy detail fetch). |
| **Now** | Touch 3 PR Review track substantially complete (slices 1–5 shipped 2.12.0–2.14.0). MCP discovery Phase 2 slice 1 shipped 2.15.0 — auto-test on add and OAuth flows are the next slices in that track. The Phase 2 profiles picker-promotion track closed for ai-editor with `kb.v1` (2.8.0); `chat_multi.v1` / `rp.v1` are deprioritized for this product (no real consumer — see *Next* row). Slot is open; candidates are listed in *Next* / *Later* below. Per the doc's own discipline (§"How to read this doc"), shipped detail lives in [CHANGELOG.md](../CHANGELOG.md), not here. |
...
- **github#27 Phase 2 — `MCP Server Discovery & Easy Configuration`** — Phase 2 **slice 1** (Smithery dynamic catalog + search/chip filter + lazy detail fetch) shipped 2.15.0 — see [CHANGELOG §2.15.0](../CHANGELOG.md). Phase 1 (curated 8-server bundled catalog) shipped 2.3.0. Open follow-up slices: **slice 2** (auto-test on add) and **OAuth flows** (formerly "Phase 1.5"). Custom catalog sources + Phase 3 self-hosted templates remain deferred.
