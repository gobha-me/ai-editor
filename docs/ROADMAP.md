...
> Last updated: 2026-05-15 (2.50.0.1 shipped — gitea#423 Anthropic-style prompt-cache breakpoint on Venice requests, in-track patch off 2.50.0). Current released: **v2.50.0** (tag fired 2026-05-14); 2.50.0.1 sub-patch does not fire the release-readiness gate per the `X.Y.Z.N` convention. Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: ICD #4 finding #3 (`resolveReviewThread` dead-code decision — promote vs demote) requires an architecture-decision session before it can ship; gitea#421, #422, #424, #425, #426 (five remaining model-behavior / token-economy issues from the 2026-05-14 AAR cohort — #423 closed by this 2.50.0.1 patch) are candidates for follow-on in-track patches or the next code minor.
