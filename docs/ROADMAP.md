...
> Last updated: 2026-05-14 (2.47.0.1 shipped — bundled docstring cleanup: 15 retrieval-module sibling-file `js/context-manager.js` references + `getToolsForProfile` registry docstring fix; absorbs the third RE-EVAL slot's doc-only content from `[Unreleased]` per Decision §14 sub-clause). Current released: **v2.47.0.1** (sub-patch under the 2.47.0 tag) · `main` HEAD: 2.47.0.1 + fresh `[Unreleased]`. Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.48.0 — shape TBD (no `[strong]`-band rows queued; the next `RE-EVAL following 2.49.0` slot is anchored 3 code minors out).
