...
> Last updated: 2026-05-14 (2.47.0 shipped — runtime regression fixes for `window.startWorkOnIssueFromList` + `Providers.enrichModels` facade gap; the pre-approved 2.47.0 docstring-cleanup row was bumped to 2.47.0.1 by user-surfaced regressions in a live `editor.gobha.ai/dev` diagnostic dump). Current released: **v2.47.0** · `main` HEAD: 2.47.0 + [Unreleased] (third RE-EVAL slot doc-only content continues to accumulate per Decision §14 sub-clause). Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.47.0.1 — the bundled docstring cleanup (sibling-file `js/context-manager.js` references across `js/intelligence/retrieval/` + stale `getToolsForProfile` docstring in `js/tools/registry.js`).
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.47.0.1 — sibling-file `js/context-manager.js` reference cleanup across `js/intelligence/retrieval/` + stale `getToolsForProfile` docstring fix**. Pre-approved at the third RE-EVAL slot; queue-jumped by 2.47.0's runtime regression fixes. ~12 files surfaced by the 2.46.0 audit pass (`index.js`, `pipeline.js`, `bm25-indexer.js`, `loader.js`, `ledger-consumer.js`, `measurement.js`, `strategies/semantic.js`, `strategies/thematic.js`, `test-corpus.js`, `manager.js:4–5`, plus `walker.js`, `comparison.js`, `wiring.js`, `ingest-controller.js`, `embedder.js`) + `js/tools/registry.js:253–271` docstring rewrite. Bundleable as one sub-patch PR. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. | `[strong] [S]` |
| **Next (2.48.0+)** | Shape TBD by what 2.47.0.1 surfaces. Inventory candidates if a new sweep wave opens: dependent on the next audit pass. Touch 3 Window v2 / Sessions and MCP OAuth Phase 2 remain `[medium]`-band candidates awaiting DESIGN-doc authoring. | `[fuzzy]` |
| ~~**Just shipped (2.47.0)**~~ | ~~**Runtime regression fixes — `window.startWorkOnIssueFromList` (issue-list Start button) + `Providers.enrichModels` (core.js facade gap).**~~ ✅ shipped 2026-05-14. Two stale-reference regressions surfaced from a user diagnostic dump on a live 2.46.0 session. [`js/app.js`](../js/app.js) lines 836–843 + [`js/core.js`](../js/core.js) lines 1425–1438; two regression tests added at [`tests/test-app-issue-list-wiring.mjs`](../tests/test-app-issue-list-wiring.mjs) and [`tests/test-core-providers-facade.mjs`](../tests/test-core-providers-facade.mjs). Pre-approved 2.47.0 docstring-cleanup row re-scoped to 2.47.0.1. | `[strong] [S]` (shipped) |
...
**~~Third re-eval slot: `RE-EVAL following 2.46.0`~~** — ✅ ran 2026-05-14 (this re-eval). Doc-only — **no version bump** (policy refinement adopted this slot; accumulates in `[Unreleased]`). Deliverables: (a) **third ICD** authored at [`docs/ICD-tool-registry.md`](ICD-tool-registry.md) — Tool registry admission contract per target #3 (`LEGAL_GROUP_TAGS` derivation at 2.34.0 + `roles: 'all'` / `allowed_groups: '*'` carve-outs + `'full'` legacy-bypass tag); 11 exports, 5 classification axes, three carve-outs documented. (b) **ARCHITECTURE.md catch-up sync** from 2.44.0 → 2.46.0 (header bump + new tools-registry ICD pointer + ICD-hierarchy row + Slot & Event Registries section-header range fix). (c) **Paper-half roadmap output:** policy refinement (re-eval slots don't consume a version), cadence-language tightening, Now/Next/Later restructure, Decision §14 sub-clause. **Code-aware findings → 2.47.0+:** one drift item — `ToolRegistry.getToolsForProfile` docstring claims a retired deprecation shim "is preserved below" (registry.js:265–267) — `[strong] [S]` row for the next in-track patch.
...
| **2.47.0.1 — sibling-file `context-manager.js` reference cleanup + stale `getToolsForProfile` docstring fix** | [`docs/ICD-tool-registry.md`](ICD-tool-registry.md) §"Code-aware findings" + the 2.46.0 audit pass's out-of-scope finding | `[strong] [S]` for both — bundleable (both are docstring-shape fixes, no behavior change); ~15 retrieval-module files + 1 registry-module file in one sub-patch PR |
...
Translation post-2.47.0: the **audit-sweep arc closed** at 2.44.0 (22 entries resolved across 11 system buckets between 2.33.0 and 2.44.0); the **second re-eval** ran at 2.45.0 and surfaced one `[strong]` row for 2.46.0 (shipped); the **third re-eval** ran 2026-05-14 (doc-only, accumulating in `[Unreleased]`; no version bump) and surfaced one `[strong]` row originally targeting 2.47.0. **A user-surfaced diagnostic dump intervened**: two runtime stale-reference regressions (`window.startWorkOnIssueFromList` + `Providers.enrichModels` facade gap) took precedence and shipped as 2.47.0 (the pre-approved docstring cleanup re-scoped to 2.47.0.1). The two named near-term features (Window v2 / Sessions, MCP OAuth) stay `[medium]` with DESIGN-doc authoring as their promotion preconditions.
