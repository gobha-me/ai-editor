...
| **Now** | **Slot reopened.** 1.14.0 just shipped (chat.v1 profile data + `resolveProfile` inheritance helper, PR #322 / commit `17c8e35`). Latest tagged on origin: `v1.14.0`. The 1.10–1.14 stretch is fully tagged. |
| **Next** | **Path to 2.0.0** — 1.14.0 just shipped (chat.v1 + `resolveProfile` inheritance helper, PR #322, tag `v1.14.0`). Five minors + one patch from there to the load-bearing flip; pinned in §"2.X path" below. Parallel candidates: LLM-authored automation Phase 1, Plugin Discoverability minor. |
...
| **1.14.1** *(patch — `v1.14.0` is already tagged, so this is a follow-up patch; could promote to 1.15.0 if the equivalence test grows teeth and bundles with ledger Phase 1)* | `coder-v1.js`: set `base: 'chat.v1'`; trim overrides matching base (`total_tokens`, `system_reserve`, `history_reserve` in `budget`; `chunkers`, `metadata_extensions` in `retrieval`). Keep the actual overrides (`output_reserve: 8000`, `memory_reserve: 1500`, `preserve_recent: 24`, full `tools.static` array, `task_ledger.capacity: 500`, `novelty_threshold: 0.3` in two places). New `tests/test-profile-resolution.mjs` proves `resolveProfile(CODER_V1_TRIMMED, lookup) ≡ CODER_V1_PRE_TRIM` field-by-field. | None — data-only. The test is the proof that subsequent slices can rely on resolution being sound. | No. |
| ~~**1.15.0**~~ *(✅ shipped — re-admission markers + capacity cap. The novelty algorithm + Composer step 6.5 already shipped in 1.4.18 with `task_ledger: null` at the production call site; this slice flipped that null on at [`js/intelligence/retrieval/manager.js:651`](../js/intelligence/retrieval/manager.js), pinned the marker format including `; ~<tokens> tokens`, added capacity-cap enforcement in `_spillIfAtCapacity`, filtered marker chunks from `rollupToFiles`, and added `scoreNovelty` as a public alias for `_computeNovelty`.)* | Task ledger Phase 1 — populate `admissions`/`exclusions` arrays at runtime; implement `scoreNovelty(currentQuery, priorAdmission)` per [`docs/DESIGN-profiles.md`](DESIGN-profiles.md) lines 184–219 (cosine + keyword + time-decay signals); suppression marker `[Already admitted: {chunk_id} — see turn {prior.turn_id}; ~{prior.tokens} tokens]` per design lines 213–219. Wires into the retrieval Composer so a follow-up turn that already admitted a chunk gets a marker instead of re-paste. | Retrieval Composer gains a ledger-consultation step before each admission. | **Yes** — follow-up turns that re-touch already-admitted chunks see a marker instead of re-pasted content. Token savings on multi-turn coder workflows. |
| **1.16.0** | `resolve.js` rewires: `resolveCompressionConfig(profileName)` (was `(role)`) — returns the resolved profile's compression slice. Coder still gets its rules; chat surfaces now use `chat.v1.compression` (Rule 5 only, **`preserve_recent: 4`** instead of the `rule5_only_shim`'s `24`). Tiny `roleToProfileName(role)` translator inside `resolve.js` keeps callers role-keyed until 1.20.0. | Compression resolver reads from a *resolved* profile (deep-merge consulted at lookup). | **Yes** — chat surfaces (`role !== 'coder'`) drop `preserve_recent` from 24 to 4. Reconciles the divergence noted in [`chat-v1.js:82–89`](../js/profiles/chat-v1.js). Needs long-chat regression test before tagging. |
| **1.17.0** | Memory subsystem (`js/intelligence/memory/`) reads from `profile.memory` instead of static config. `default_scope`, `propose_after_n_turns`, `capacity_warnings` all sourced from the resolved profile via the same `resolveMemoryConfig(profileName)` shape. | Memory subsystem joins compression on the resolved-profile lookup pattern. | Likely no — coder.v1 keeps `default_scope: 'session'` for now (a future decision moves it to `'workspace'`); chat surfaces start using `'user'` as the design intends. |
| **1.18.0** | Tools subsystem reads from `profile.tools.static`. Removes the three direct `CODER_V1` imports at [`js/chat/handlers.js:47, 779, 780, 803`](../js/chat/handlers.js). New `resolveTools(profileName)` helper. | Tools subsystem on the same lookup pattern. Cleans up the cross-cutting direct-import smell. | No — same static set is admitted, just via the resolver. |
| **1.19.0** | Retrieval Composer reads `profile.retrieval` — `strategy_weights`, `collections`, `memory_collections`, `novelty_threshold`. Composer landed in 1.5.0 already; this rewires the lookup, not the algorithm. | Last subsystem rewire. **Profiles are now load-bearing internally** even though the Settings surface is still role-keyed. | No — same retrieval behavior per role (chat.v1 weights match chat surfaces; coder.v1 weights match coder). |
...
> **Sandbox / LLM-authored automation — single feature minor.** New `submit_script_for_approval` tool (`readOnly: true`), modeled byte-for-byte on the 1.10.0 `submit_plan_for_approval` lifecycle ([`js/tools/plan-mode.js`](../js/tools/plan-mode.js) + [`js/chat/plan-approval-card.js`](../js/chat/plan-approval-card.js)). Handler returns a `Promise` that resolves only after user approves in an inline approval card; resolution path runs the script in a sandboxed Web Worker that cannot reach `window.AIEditor`, the tool registry, the network, or any write API. Tier 0 scope = read-only filesystem walk over the loaded project (uses existing `Git.getFile` / `Git.getFileTree`). Cost-saved estimate per invocation against a counterfactual `read_file` loop, surfaced in the cost dashboard. Full design in [`docs/DESIGN-llm-authored-automation.md`](DESIGN-llm-authored-automation.md). **Independent of the Profiles arc** — gated only on Plan Mode (already shipped 1.10.0). Phase 2 (graduation measurement; Tier 1 HTTP allowlist) parks behind real Tier-0 usage data.
