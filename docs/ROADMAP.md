...
> Last updated: 2026-04-30 · Current released version: **1.3.6** · Authoring branch: `feat/roadmap-1.x`
...
- **1.3.8: Connections panel (Touch 2 net-new surface).** Currently buried in Settings → Connections; redesign as N-of-each-provider per `docs/design/touch-2-facelift/project/connections.jsx`. Per-provider list (multiple GitHub accounts, self-hosted Gitea, GitLab) with "Add another" + status (ok/warn/error + last-sync + account + repo count). Aggregated repo picker shows provenance ("personal/foo (GitHub: jeff)" vs "work/bar (GitHub: jsmith-acme)"). The original Settings mockup had a single dropdown — real users have 2-4 providers. This is a **data-model change** (`State.settings.connections[]` already exists; the UI wasn't keeping up) more than a UI change.
