...
> Last updated: 2026-05-02 · Current released version: **1.4.12** · Authoring branch: `claude/jolly-volhard-998f9f`
...
| Phase | Track | Status |
|---|---|---|
| **Now** | Retrieval chunker stream in-flight (1.4.9 foundation ✓ → 1.4.10 ProseChunker ✓ → 1.4.11 CodeChunker ✓ → 1.4.12 ConversationChunker ✓) — Phase 1 (1.5.0) is the promotion milestone | Tools track fully shipped through 1.4.8 (Phase 1 at 1.4.0 with 79.5% token reduction live; 1.4.1 semantic `find_tool` ✓, 1.4.2 MCP bridge ✓, 1.4.4 workspace-scoped settings ✓, 1.4.5 test-driven loop ✓, 1.4.6 scan-driven CI logs ✓, 1.4.7 ghost text ✓, 1.4.8 tuning + LRU eviction ✓). 1.4.9 ✓ shipped the Retrieval data foundation (ChunkRef contract + ChunkID hash + module scaffolding); 1.4.10 ✓ added ProseChunker and pinned the `Chunk`/`Chunker`/`ChunkerInput` contract; 1.4.11 ✓ added CodeChunker (Phase 1 heuristic regex for JS/TS/Python; AST deferred to 1.5.5); 1.4.12 ✓ adds ConversationChunker (1 turn = 1 chunk over a JSON-serialized HistoryTurn[]). |
| **Next** | 1.4.13 — Structured-chunker scope decision then chunker (per-record over JSON / JSONL — formats v1 covers TBD), then Retrieval Phase 1 (1.5.0) StructureExtractor, Semantic + Structural strategies, Composer, ledger consumer, migration off `js/context-manager.js` | Designed; foundation + Prose + Code + Conversation chunkers laid in 1.4.9 / 1.4.10 / 1.4.11 / 1.4.12. Structured needs a brief scope decision (which formats v1 covers) before implementation — "per record" is format-specific in a way Conversation's "per turn" is not. |
...
- **1.4.13:** Retrieval Phase 1 — **StructuredChunker** (PR 5 of 1.5.0). Per `docs/DESIGN-retrieval.md` §"Chunker" the chunker emits per record over top-level keys / array elements. Prerequisite scope decision before implementation: which formats v1 covers (JSON only? include JSONL? skip CSV / YAML / TOML until a real consumer asks?) and how nested-record granularity maps to byte_range. Conversation's "1 turn = 1 chunk" is unambiguous; "per record" is format-specific. Track the decision via a short pre-PR note (or design-doc paragraph) before opening the implementation PR.
