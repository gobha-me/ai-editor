...
> Last updated: 2026-05-01 · Current released version: **1.3.13** · Authoring branch: `feat/roadmap-1.x`
...
- **1.3.15 [PLANNED]:** System-prompt admission alignment — prereq for meta-tools (1.3.16). Replaces the hardcoded ~21-tool enumeration block in `js/prompts.js` `EditorPrompts.systemPrompt` (lines 26–48) with a dynamically-rendered list built from the Composer's admitted set. Without this, the model is invocation-limited to 6 tools (1.3.14 Composer) but self-described as having 21 — when asked "list tools" it recites from the prompt, not the API tool array, and discovers the gap immediately. The WORKFLOW / EFFICIENCY / EDITING blocks below need a parallel scrub since they reference unadmitted tools by name (`find_relevant_files`, `peek_project_tree`, `scratchpad_*`); 1.3.15 either rewrites those guidance blocks to be tool-agnostic or scopes them behind admission too. Discovery (1.3.16 meta-tools) needs the prompt to direct the model to *call discovery* rather than enumerate, so 1.3.15 is the natural prereq. **Touchpoints:** `js/prompts.js` lines 23–167 (the system prompt body); `js/chat/handlers.js:300-301` (passes admitted tools into `buildSystemPrompt`).
- **1.3.16 [PLANNED]:** Meta-tools (PR 3 of 1.4.0 Tools Phase 1) — `list_tool_categories`, `list_tools_by_category`, `find_tool` (categorical only; semantic in 1.4.1). Closes the `unresolved_static` gap surfaced by the 1.3.14 Composer for the three names already declared in `coder.v1.tools.static`. Discovery roundtrip becomes testable: the model calls `list_tools_by_category("file")`, gets summaries, and calls one — admission stays sticky for the rest of the task (sticky admission ships in a later patch). Lands as 1.3.16 (not 1.4.0) until the 70% token-reduction exit criterion is measurable end-to-end on real sessions.
