...
> Last updated: 2026-05-10 (post-2.9.1) · Current released: v2.9.1 · `main` HEAD: 2.9.1.
...
| Phase | Track |
|---|---|
| **Just shipped** | Shipped through v2.9.1 — see [CHANGELOG.md](../CHANGELOG.md). Recent: 2.8.0 (`kb.v1` picker promotion + per-chat profile binding); 2.9.0 (provider rate-limit pacer — Touch 3 Window v2 / Sessions hard prerequisite cleared); 2.9.1 (anti-loop refusal payload — concrete recovery candidates at streak ≥ 3). |
| **Now** | Between active tracks post-2.9.1. The Phase 2 picker-promotion track closed for ai-editor with `kb.v1` (2.8.0); `chat_multi.v1` / `rp.v1` are deprioritized for this product (no real consumer — see *Next* row). The next slot is open; candidates are listed in *Next* / *Later* below. Per the doc's own discipline (§"How to read this doc"), shipped detail lives in [CHANGELOG.md](../CHANGELOG.md), not here. |
| **Next** | **Tier 3 in-editor preview** — gate fired by Tier 2 dogfood (qwen-3-6-plus exercised the 4 new readers on a clean HTML-Games page; see [CHANGELOG §2.7.0](../CHANGELOG.md)). Sidecars + selector-shaped tools per `docs/DESIGN-preview.md` §Phased Delivery; `preview_eval` decision still open. Independent of the Profiles arc. **Profiles Phase 2 picker promotion** for `chat_multi.v1` / `rp.v1` is **deprioritized for ai-editor** — those profiles target consumers (multi-user shared chat, role-play personas) that don't exist in this product; promotion needs a different consumer (custom plugin profile or separate product surface) to earn user-observable weight. Picker-list tests stay pinned at `['chat.v1', 'coder.v1', 'kb.v1']`. **Retrieval ingest hardening** shipped through 2.2.0 (delta-indexing) + 2.4.0 (language-stats + token cap) — track closed. |
...
- **github#27 Phase 2 — `MCP Server Discovery & Easy Configuration`** — Settings UI for discovery + per-server config. Phase 1 (curated MCP server catalog) shipped 2.3.0 — see [CHANGELOG §2.3.0](../CHANGELOG.md). Medium effort, can land any time the slot is free.
