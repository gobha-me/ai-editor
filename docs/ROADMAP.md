...
| **Now** | **2.0.0 just shipped (slice 3 of path-to-2.0.0, final).** Role selector retired; one-shot `settings.role` → `settings.profile` migration runs in `loadSettings`; `Roles` namespace + `BUILTIN_ROLES` + `Roles.filterTools` + `roleToProfileName` deleted. `window.AIEditor.Roles` deprecation shim warns once + retires at 2.1.0. **Unblocks** github#24 sub-agents and Touch 3 Window v2 / Sessions. |
| **Next** | **One of the parallel 1.X tracks** — Plugin Discoverability minor (closes the three "no settings UI" rows in `docs/PLUGIN.md`), Retrieval ingest hardening (delta-indexing on branch switch + language-stats-driven ingest order; baseline already recorded), or Tier 2 of in-editor preview (gated on dogfood signal — Tier 1 shipped at 1.22.0). All independent of the (now-completed) profiles arc. |
...
| ~~**1.24.0**~~ *(✅ shipped — slice 2 of 3)* | Consumer flip: both `js/llm/api.js` filter call sites (cost baseline, legacy fallback) flipped from `Roles.filterTools(defs)` to `Profiles.filterTools(defs, getActiveProfileName(State.settings))`; `js/prompts.js` SDK addendum flipped from `role.systemPrompt` to `profile.systemPrompt`. `roleToProfileName` widened from `coder ? 'coder.v1' : 'chat.v1'` to a 5-key table mapping every legacy role to its synthetic-or-canonical profile, so the runtime path is byte-equivalent for every existing user. `Roles` import dropped from `js/llm/api.js` (unused after the flip). | Profile-keyed reads win at the admission-filter + system-prompt sites; `Roles.filterTools` orphaned in `js/core.js`. | No — byte-equivalent runtime for every existing user. |
| ~~**2.0.0**~~ *(✅ shipped — slice 3 of 3, final)* | Role selector removed. Migration script: `settings.role` → `settings.profile`, audit-logged, one-shot, extracted to `js/profiles/migration.js`. `roleToProfileName` translator deleted. `Roles.filterTools` + `BUILTIN_ROLES` + the `Roles` namespace deleted; `window.AIEditor.Roles` deprecation shim warns once + retires at 2.1.0. Picker `(use role)` sentinel retired; default profile is `chat.v1`. Plugin-dev auto-switch flipped to write `State.settings.profile = 'plugin-dev.v1'`. Removability cross-check verified live in-browser: every (legacy_role, mapped_profile) pair admits the same set under `Profiles.filterTools` as legacy `Roles.filterTools` did. | **The profile contract is the load-bearing configuration surface.** Role concept gone from the UI and from the lookup paths. | **Yes** — role selector disappears. Existing users get migrated. Behavior identical for any user who never touched the picker. Unblocks github#24 sub-agents and Touch 3 Window v2 / Sessions. |
