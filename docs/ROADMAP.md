...
> **In-editor preview & verify (Tier 3) — driveable preview, downstream.** Sidecars (Playwright per workspace), selector-shaped tools (`preview_snapshot`, `preview_click`, `preview_fill`, `preview_inspect`, `preview_screenshot`, `preview_resize`, `preview_network`-driven), and the `preview_eval` per-invocation-gate question (may never ship — selectors cover most probes). Gated on Tier 2 dogfood signal per `docs/DESIGN-preview.md` §Phased Delivery — each its own minor, earned on a measured class of bug selectors / capture cannot serve. **Independent of the Profiles arc.**
