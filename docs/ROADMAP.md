...
| **Now** | **Slot reopened.** 1.17.0 just shipped (compression resolver — first profile-arc consumer rewire). 1.14.0 / 1.14.1 / 1.14.2 / 1.15.0 / 1.16.0 / 1.17.0 sit in main untagged after `v1.14.1`. The 1.10–1.14 stretch is fully tagged through `v1.14.1`. |
| **Next** | **Path to 2.0.0** — Profiles arc continues at 1.18.0 (memory subsystem resolver). Four minors + one patch from here to the load-bearing flip; pinned in §"2.X path" below. Remaining parallel candidates: Plugin Discoverability minor, Retrieval ingest hardening, **In-editor preview & verify Tier 1** (design landed 2026-05-08; see [`docs/DESIGN-preview.md`](DESIGN-preview.md) and §"Parallel 1.X tracks" below). |
...
| **1.18.0** *(was 1.17.0)* | Memory subsystem (`js/intelligence/memory/`) reads from `profile.memory` instead of static config. `default_scope`, `propose_after_n_turns`, `capacity_warnings` all sourced from the resolved profile via the same `resolveMemoryConfig(profileName)` shape. | Memory subsystem joins compression on the resolved-profile lookup pattern. | Likely no — coder.v1 keeps `default_scope: 'session'` for now; chat surfaces start using `'user'` as the design intends. |
| **1.19.0** *(was 1.18.0)* | Tools subsystem reads from `profile.tools.static`. Removes the three direct `CODER_V1` imports at [`js/chat/handlers.js`](../js/chat/handlers.js). New `resolveTools(profileName)` helper. | Tools subsystem on the same lookup pattern. Cleans up the cross-cutting direct-import smell. | No — same static set is admitted, just via the resolver. |
| **1.20.0** *(was 1.19.0)* | Retrieval Composer reads `profile.retrieval` — `strategy_weights`, `collections`, `memory_collections`, `novelty_threshold`. Composer landed in 1.5.0 already; this rewires the lookup, not the algorithm. | Last subsystem rewire. **Profiles are now load-bearing internally** even though the Settings surface is still role-keyed. | No — same retrieval behavior per role. |
| **1.21.0** *(was 1.20.0)* | Settings UI — profile picker. New picker reading from `js/profiles/registry.js` (tiny lookup exposing `chat.v1` + `coder.v1`). Picker sits alongside the role selector; both write to the same internal state via the `roleToProfileName` translator. **The picker exists; role still wins if untouched.** | Settings surface gains a parallel surface; role↔profile translator carries the duality. | **Yes** — new picker UI visible in Settings. Role selector unchanged. Default behavior unchanged. |
