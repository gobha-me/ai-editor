...
| **Now** | **Slot reopened.** 1.16.0 just shipped (LLM-authored automation Phase 1 — Tier 0 in-browser Worker + Plan-Mode-shaped approval card). 1.14.0 / 1.14.1 / 1.14.2 / 1.15.0 / 1.16.0 sit in main untagged after `v1.14.1`. The 1.10–1.14 stretch is fully tagged through `v1.14.1`. |
| **Next** | **Path to 2.0.0** — Profiles arc resumes at 1.17.0 (compression resolver — was 1.16.0 before the parallel 1.X track interleaved per §"Parallel 1.X tracks"). Five minors + one patch from here to the load-bearing flip; pinned in §"2.X path" below. Remaining parallel candidates: Plugin Discoverability minor, Retrieval ingest hardening. |
...
| **1.17.0** *(was 1.16.0 pre-interleave)* | `resolve.js` rewires: `resolveCompressionConfig(profileName)` (was `(role)`) — returns the resolved profile's compression slice. Coder still gets its rules; chat surfaces now use `chat.v1.compression` (Rule 5 only, **`preserve_recent: 4`** instead of the `rule5_only_shim`'s `24`). Tiny `roleToProfileName(role)` translator inside `resolve.js` keeps callers role-keyed until 1.21.0. | Compression resolver reads from a *resolved* profile (deep-merge consulted at lookup). | **Yes** — chat surfaces (`role !== 'coder'`) drop `preserve_recent` from 24 to 4. Reconciles the divergence noted in [`chat-v1.js:82–89`](../js/profiles/chat-v1.js). Needs long-chat regression test before tagging. |
