...
| ~~**Merge Conflict Resolver**~~ *(✅ slices 1+2 shipped 2.18.0–2.19.0; slice 3 implemented + parked in stash pending the next minor slot)* | `merge-conflict.jsx`, `Facelift.html#merge` section | New surface, minor-sized | Slice 1 (2.18.0): three-pane + Take theirs/ours + push resolved. Slice 2 (2.19.0): Take both + conflict minimap + GitLab/Local capability flag. **Slice 3 (AI resolve per hunk) — implemented locally** but stashed when 2.20.0 zip-flow took the slot; will unstash and ship as a follow-up minor (stash-pop conflict is limited to `js/version.js` and a fresh CHANGELOG entry). Designer confirmed feasible in pure browser. Independent of Sessions. |
