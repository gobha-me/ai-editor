...
| **Next** | **Path to 2.0.0** — Profiles arc continues at 1.18.0 (memory subsystem resolver). Four minors + one patch from here to the load-bearing flip; pinned in §"2.X path" below. Remaining parallel candidates: Plugin Discoverability minor, Retrieval ingest hardening. |
