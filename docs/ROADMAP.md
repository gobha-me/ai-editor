...
> Last updated: 2026-05-09 (post-2.1.1) · Current released: v2.1.1 · `main` HEAD: 2.1.1.
...
| **Profiles Phase 2 picker promotion** | — | Promote `chat_multi.v1` / `rp.v1` / `kb.v1` from `SYNTHETIC_ENTRIES` (lookup-only) back to `ENTRIES` (picker-visible) when each profile carries *something a user can observe choosing it for*. The headline lever is profile-specific `systemPrompt` addenda mirroring 1.23.x's [`plugin-dev.v1`](../js/profiles/plugin-dev-v1.js) precedent (a `kb.v1` with *"answer only from attached_docs, cite line ranges, no edits"* actually behaves differently). Trigger: any one of those addenda lands → promote that one profile (granular promotion is fine; doesn't have to be all-or-nothing). Picker-list assertions in [`tests/test-profiles-registry.mjs`](../tests/test-profiles-registry.mjs) and [`tests/test-profile-filter-tools.mjs`](../tests/test-profile-filter-tools.mjs) are pinned to `['chat.v1', 'coder.v1']` precisely so the gate is load-bearing — promoting requires updating those assertions, which surfaces in PR review. Per-profile rationale (cost-quality / behavior delta vs. `chat.v1`) lives next to each profile in [`js/profiles/registry.js`](../js/profiles/registry.js) `SYNTHETIC_ENTRIES` rationale block. **Custom plugin profiles inheriting `base: 'rp.v1'`** etc. unlock with the Phase 4 authoring API (parked); that's a different consumer for the same data files. |
