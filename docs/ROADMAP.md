...
**Re-evaluation cadence: every 3 minors** (see §"Re-evaluation cadence" below). The cadence keeps the sliding window aligned: content promotes (fuzzy→medium→strong) as milestones approach; content demotes or relocates as milestones slip.
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.47.0 — `RE-EVAL following 2.47.0`** is the next scheduled re-eval slot (3-minor cadence). ICD-backfill target #3 (Tool registry admission contract — `LEGAL_GROUP_TAGS` derivation + `roles: 'all'` / `allowed_groups: '*'` carve-outs) is the code-aware target. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. | `[rewrite session]` (re-eval) |
| **Next (2.48.0+)** | Shape TBD by the 2.47.0 re-eval's code-aware half. Inventory candidates if a new sweep wave opens: dependent on the next audit pass. One known candidate already in-hand: the ~12 sibling-file `js/context-manager.js` stale references in `js/intelligence/retrieval/` surfaced by the 2.46.0 audit pass (`index.js`, `pipeline.js`, `bm25-indexer.js`, `loader.js`, `ledger-consumer.js`, `measurement.js`, `strategies/semantic.js`, `strategies/thematic.js`, `test-corpus.js`, `manager.js:4–5`) — deferred from 2.46.0 to preserve the re-eval's "same module" scoping. | `[fuzzy]` until re-eval picks |
| ~~**Previous (2.46.0)**~~ | ~~**2.46.0 — retrieval Composer stale docstring fix.**~~ ✅ shipped 2026-05-14. Single-file fix at [`js/intelligence/retrieval/composer.js`](../js/intelligence/retrieval/composer.js) lines 31–35 — replaced "No runtime wire-up" (stale since 1.5.14) with "Production wiring (since 1.5.14)" + forward-pointer to [`ICD-intelligence-composers.md`](ICD-intelligence-composers.md). Audit pass: lines 31–34 were the only drift in `composer.js`. ~12 sibling files queued for a future re-eval slot (see Next above). | `[strong]` (shipped) |
...
Per methodology §Phase 3, every 3 code minors past the next-N window get a re-evaluation slot. Re-eval items appear inline in forward cadence here, distinguished from code milestones by the `[rewrite session]` tag (no band label — re-eval isn't code work). The first re-eval slot is anchored at the next milestone boundary post-2.38.0.
...
**~~Second re-eval slot: `RE-EVAL following 2.44.0`~~** — ✅ ran 2026-05-14 as the 2.45.0 doc-only PR. Deliverables shipped: (a) **ARCHITECTURE.md catch-up sync** from 2.41.0 → 2.44.0 (4-minor delta: 2.42 first re-eval + ICD, 2.43 rail-views migration, 2.44 audit-sweep wave close) — added DomBindings (2.44.0.1) + TabActivationRegistry (2.44.0.2) + Chat tool-name pin (2.44.0.0) + `managers/` retirement (2.44.0.3); ICD-backfill row updated. (b) **Second ICD** authored at [`ICD-intelligence-composers.md`](ICD-intelligence-composers.md) — Tools + Retrieval Composer seam contract per target #2; 5 classification axes (Source/Budget/Authorization/Overflow/Diagnostics) over the two pure-function Composers; cross-references ICD-chat-handlers §"Composer-vs-non-Composer path drift" for the system-prompt parallel. (c) **Paper-half roadmap output:** Touch 3 Window v2 / Sessions and github#27 OAuth flows decisions; one `[strong] [S]` row for 2.46.0 (retrieval Composer stale docstring — the only drift surfaced).

**Next re-eval slot: `RE-EVAL following 2.47.0`** — anchored per the 3-minor cadence. ICD-backfill target #3 (Tool registry admission contract — `LEGAL_GROUP_TAGS` derivation at 2.34.0 + `roles: 'all'` / `allowed_groups: '*'` carve-outs) is the code-aware target.
...
| ~~**2.46.0 — retrieval Composer stale docstring fix**~~ | [`ICD-intelligence-composers.md`](ICD-intelligence-composers.md) §"Code-aware findings" | ✅ shipped 2.46.0 — docstring lines 31–35 replaced + audit pass confirmed lines 31–34 were the only drift in `composer.js`; ~12 sibling-file `context-manager.js` references deferred to a future re-eval slot |
| Touch 3 Window v2 / Sessions | None — `docs/DESIGN-sessions.md` will need authoring | `[medium]` retained at the 2.45.0 re-eval — 2.0.0 dependency cleared (the Touch 3 design's hard prerequisite); 6 architecture questions parked in [`discussion/touch-3-window-v2-sessions.md`](discussion/touch-3-window-v2-sessions.md). Promotion to `[strong]` blocked on DESIGN-sessions.md authoring slot. |
| MCP discovery Phase 2 OAuth flows ([github#27](https://github.com/gobha-me/ai-editor/issues/27)) | None — `docs/DESIGN-mcp-oauth.md` will need authoring | `[medium]` retained at the 2.45.0 re-eval — security surface warrants paper-first contract; one external request is enough pull given the OAuth contract implications. DESIGN-mcp-oauth.md authoring slot queued (no specific milestone target yet). |

Translation post-2.45.0: **the audit-sweep arc closed** at 2.44.0 (22 entries resolved across 11 system buckets between 2.33.0 and 2.44.0); the second re-eval ran at 2.45.0 and surfaced exactly **one** `[strong]` row for the next code minor (2.46.0 — retrieval Composer docstring fix). The two named near-term features (Window v2 / Sessions, MCP OAuth) stay `[medium]` with DESIGN-doc authoring as their promotion preconditions.
