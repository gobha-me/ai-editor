...
> Last updated: 2026-04-30 · Current released version: **1.3.3** · Authoring branch: `feat/roadmap-1.x`
...
- **1.3.6: Top bar Restructure (Touch 2 LOCKED).** Rebuild the top bar to the LOCKED-for-ship Restructure layout from `docs/design/touch-2-facelift/project/topbar.jsx`: ~56px tall; brand mark + repo/branch indicator (with ahead/behind counts) + ⌘K command surface + token-usage pill + 3-icon row (Settings / Help / Debug). Replaces the current 5-unlabeled-icon junk drawer (revert / settings / plugins / models / help). Revert moves to the editor toolbar where the diff lives; plugins moves to Settings; model picker moves to chat compose. One component, themed via `--tk-*` (no per-theme variants of the component itself). Reads from the 1.3.5 token contract; visual lift comes from the contract, not from the component re-implementing palette decisions. The pushback memo's strongest LOCKED item.
