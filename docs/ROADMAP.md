...
> Last updated: 2026-05-01 · Current released version: **1.3.8** · Authoring branch: `feat/roadmap-1.x`
...
- **1.3.9: Debug slide-out (Touch 2 net-new surface).** New right-edge slide-out per `docs/design/touch-2-facelift/project/debug.jsx`: Logs (level filter), Connections (per-provider health + last error), Indexer (queue, paused state), AI (request log with tokens / latency / cost), Plugins (warnings, lifecycle errors). "Copy diagnostic bundle" button — pre-formatted tarball-equivalent JSON the user can paste into a bug report. Removes the "open devtools or file a ticket" failure mode for non-technical users. Same slide-out pattern as Settings so it doesn't fight chat for the main column.
