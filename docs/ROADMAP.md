...
| **Next** | **One of the remaining parallel 1.X tracks** — Retrieval ingest hardening (delta-indexing on branch switch + language-stats-driven ingest order; baseline already recorded), or Tier 2 of in-editor preview (gated on dogfood signal — Tier 1 shipped at 1.22.0). Both independent of the (now-completed) profiles arc. |
| **Later** | Open issues #38, #37 (Phase 2), #27, #24, #18 (see *Known open issues* below) — none currently on the active track. |
...
| **2.2.0** | Phase 2 | `chat_multi.v1`, `rp.v1`, `kb.v1` profiles registered (`DESIGN-profiles.md` line 455). Inheritance through one level (base → leaf); per-profile worked-example test fixtures. Surface count goes from 1 to 4. **Also:** retire the `window.AIEditor.Roles` deprecation shim (was 2.1.0 in the 2.0.0 changelog). |
...
- **github#38 — `Approval card surfaces 'Edit rejected' with no tool name, args, or diff context`** — Filed 2026-05-09 from a HTML-Games dogfood session: a write-class tool (likely `edit_file`) hit the approval gate with no rendering of tool name or args, so the user rejected twice on "commit your last change and open a PR" before bypassing with a narrower prompt. Fix shape: the approval card should mirror the per-tool-call chat header (tool name + arg summary, plus a diff preview for `edit_file` / `write_file`). Independent of the profiles arc; small UX patch. See `project_approval_card_visibility.md` (memory).
