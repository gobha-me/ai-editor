...
- **1.3.7: Settings sidebar Restructure (Touch 2 LOCKED).** Replace the 11-tab horizontal strip in `html/modals.html` with a vertical sidebar grouped Workspace / AI / App per `docs/design/touch-2-facelift/project/settings.jsx`. Scales to 30+ items without redesign (current strip already overflows on 14"). ⌘K-search-settings comes free with the search infrastructure 1.3.6 lands. Both themes ship as the same component via `--tk-*`. Establishes the sidebar pattern that 2.0's profile picker inherits. **Power users see ⌘K + the new sidebar; newcomers see the same surface but with grouped sections instead of horizontal scroll.** Touch 2 PUSH: *"tabs in Settings are dead. Stop treating them like they have a future."*
