...
> Last updated: 2026-05-22 (**v2.93.0 shipped** — agent-loop tuning cohort: `find_relevant_files` auto-bootstraps the indexer on cold sessions ([gitea#516](https://git.gobha.me/xcaliber/ai-editor/issues/516)); same-tool streak guard holds on legit paging/narrowing exploration ([gitea#517](https://git.gobha.me/xcaliber/ai-editor/issues/517))). Per-minor detail in [CHANGELOG.md](../CHANGELOG.md); this doc commits to forward cadence, not ship history.
>
> **Next forward step:** `RE-EVAL following 2.73.0` (overdue by 7 minors at v2.93.0; cadence-rule re-examination is the paper-half agenda item — the 5-slot overrun pattern visible at slots #6/#10/#11 suggests the every-3-minor rule itself needs weighing).
...
**Next re-eval slot: `RE-EVAL following 2.73.0`** — anchored per the 3-code-minor cadence-from-anchor rule (3 code minors past 2.70.0). Overdue by 7 minors at v2.93.0; agenda includes (1) paper-half cadence-rule re-evaluation and (2) ARCHITECTURE.md catch-up sync from 2.86.0 forward.
...
**Current state (v2.93.0).** No `[strong]`-band milestones are queued in the next-3 window. Current queue + contract status:

| Item | Band | Contract | Status |
|---|---|---|---|
| `RE-EVAL following 2.73.0` | `[rewrite session]` | Plinth methodology + the 9 established ICDs | Queued (doc-only; overdue by 7 minors) |
...
**All `[strong]`-band milestones shipped 2.42.0 → 2.93.0 had on-hand contracts** — each ship's CHANGELOG entry names its ICD or DESIGN-doc anchor; see [CHANGELOG.md](../CHANGELOG.md) for per-ship detail.
