...
> Last updated: 2026-05-14 (2.45.0 re-eval slot ran — second pass under the methodology adopted 2026-05-12; doc-only minor). Current released: **v2.45.0** · `main` HEAD: 2.45.0 + [Unreleased]. Recent shipped work: see [CHANGELOG.md](../CHANGELOG.md). The 2.33.0 → 2.44.0 audit-sweep arc closed at 2.44.0 with 22 inventory entries resolved across 11 system buckets; the inventory at [`audit-2026-Q2/inventory.md`](audit-2026-Q2/inventory.md) remains live as historical record + queue for the next sweep wave. Next forward step: 2.46.0 code minor — shape picked by the just-completed re-eval (one `[strong]` row queued: retrieval Composer stale docstring fix).
...
| Phase | Track | Band |
|---|---|---|
| **Now (next up)** | **2.46.0 — retrieval Composer stale docstring fix.** Single-file `[strong] [S]` row surfaced by the 2.45.0 re-eval's code-aware half (see [`ICD-intelligence-composers.md`](ICD-intelligence-composers.md) §"Code-aware findings"): [`js/intelligence/retrieval/composer.js`](../js/intelligence/retrieval/composer.js) lines 31–34 still claim "exported but not yet called by find_relevant_files" — stale since 1.5.14 retired legacy `context-manager.js`. Audit the rest of the module's docstrings for similarly stale 1.4.x-era claims. Folds into the next in-track patch. | `[strong]` |
| **Next (2.47.0)** | `RE-EVAL following 2.47.0` is the next scheduled re-eval slot. ICD-backfill target #3 (Tool registry admission contract — `LEGAL_GROUP_TAGS` derivation + `roles: 'all'` / `allowed_groups: '*'` carve-outs) is the code-aware target. Touch 3 Window v2 / Sessions and github#27 OAuth flows remain `[medium]` until their DESIGN docs land. | `[rewrite session]` (re-eval) |
| **Next-after (2.48.0+)** | Shape TBD by the 2.47.0 re-eval. Inventory candidates if a new sweep wave opens: dependent on the next audit pass. | `[fuzzy]` until re-eval picks |
...
| **2.46.0 — retrieval Composer stale docstring fix** | [`ICD-intelligence-composers.md`](ICD-intelligence-composers.md) §"Code-aware findings" | ✅ present — single-file `[strong] [S]` row; folds into the next in-track patch |
