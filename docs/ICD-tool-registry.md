...
> **📎 Post-supersedence note — non-admission registration facets continue to evolve.** The `ToolDefinition` descriptor passed to `ToolRegistry.register()` is the same shape as pre-2.54.0 minus the retired `roles` / `_registeredRoles` fields, and is still the registration-site contract for tool authors. Subsequent additions land here without re-opening the admission ICD: **2.71.0** added an optional `cache: 'by-args' | 'never'` field to retire the cache-classification whack-a-mole pattern across 4 prior recurrences (gitea#301 / github#39 / 2.10.0 Tier 3a / gitea#472). The lifting decision lives next to the tool author at registration rather than one-or-two files away in `tool-classifications.js`; the cache axis itself is documented at [`docs/ICD-chat-handlers.md`](ICD-chat-handlers.md) §"The five axes › Cache axis — dup-detection" (the runtime consumers via `isStatefulRead(name)` / `getStatefulReadToolsLive()` accessors in [`js/chat/tool-classifications.js`](../js/chat/tool-classifications.js) union the legacy `STATEFUL_READ_TOOLS` const with the registry-driven `cache: 'never'` set). The lint guard [`tests/test-tool-cache-classifications.mjs`](../tests/test-tool-cache-classifications.mjs) source-scans `js/tools/*.js` registration blocks and requires explicit classification for stale-prone tool-name shapes (`list_*` / `find_*` / `get_*` / `*_status` / `*_logs`) via a `STALE_PRONE_NAME_ALLOWLIST` escape hatch. **2.78.0** added the **T1 tool-authored failure shape contract** — every tool-side rejection in the 11-handler `T1_CONFORMANT_TOOLS` cohort (`edit_file`, `write_file`, `read_lines`, `search_in_files`, `get_ci_status`, `wait_for_ci`, `get_ci_logs`, `read_plugin_source`, `write_plugin_source`, `run_plugin`, `run_code`) carries a `code:` field from a `VALID_CODES` closed set alongside the existing human-readable `error:` narration. The `code:` field choice aligns tool-throw failures (the registry catch block at [`js/tools/registry.js`](../js/tools/registry.js) already surfaces `EditorError` throws as `{error: msg, code: identifier}`) with tool-return failures on one envelope shape — pinned in [`docs/DESIGN-tools.md`](DESIGN-tools.md) §"Tool-authored failure shape contract" as the ai-editor concrete implementation of the design's "`error` (or `failure_code`)" clause. Cross-reference to [`docs/DESIGN-agent-loop.md`](DESIGN-agent-loop.md) §"The Authorship Rule" — tool-authored failure shapes are carried inside `SuccessEnvelope` at the loop's level (the tool ran; the loop wraps its return unchanged); the loop does not author them. The lint guard [`tests/test-tool-failure-shapes.mjs`](../tests/test-tool-failure-shapes.mjs) source-scans `js/tools/*.js`, resolves each `register('NAME', HANDLER, ...)` to its handler body (handles both inline-arrow and named-function-reference patterns), asserts every `return {... error: ...}` site in a `T1_CONFORMANT_TOOLS` handler also carries a `code:` field from `VALID_CODES`. Subsequent additions land their codes in `VALID_CODES` without sweeping the other handlers' return sites (2.81.0 added `'search_truncated'` for `search_in_files` without graduating that tool to `T1_CONFORMANT_TOOLS`; 2.84.0 added `'uncommitted_changes'` for `create_pull_request` likewise — pending graduation in follow-on PRs). **2.76.0** added the plan-mode dispatch-side gate `checkPlanModeAccess(name)` documented at §"Per-export contract" below + the `side_effects` classification source moved out of `catalog.js` to [`js/intelligence/tools/side-effects.js`](../js/intelligence/tools/side-effects.js) to break the registry↔catalog cycle (16 missing classifications added; new `PLAN_MODE_SESSION_WRITE_ALLOWLIST` for session-local writes). Future non-admission `ToolDefinition` extensions follow the same pattern: declare next to the tool, surface the question to authors via a lint guard, cross-reference back here. **This ICD's scope (admission contract) stays superseded — it does not absorb the new facets.**
