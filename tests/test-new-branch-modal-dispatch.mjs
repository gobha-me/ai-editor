/**
 * Dispatcher tests for js/ui/branch.js#mountNewBranchModal — Phase 2a of the
...
const { mountNewBranchModal } = await import('../js/ui/branch.js');
...
function makeBtn({ action, inScope }) {
    ...
    btn.closest = (sel) => {
        ...
        if (sel === '#newBranchModal') return inScope ? {} : null;
        return null;
    };
    return btn;
}
...
test('mountNewBranchModal installs exactly one document click listener', () => {
    assert.equal(_clickListenerCount, 1);
});

test('data-action="closeNewBranchModal" inside #newBranchModal → onClose fires', () => {
    resetSpy();
    _capturedListener(makeClick({ action: 'closeNewBranchModal', inScope: true }));
    assert.equal(_spy.close, 1);
    assert.equal(_spy.create, 0);
});

test('data-action="createNewBranch" inside #newBranchModal → onCreate fires', () => {
    resetSpy();
    _capturedListener(makeClick({ action: 'createNewBranch', inScope: true }));
    assert.equal(_spy.create, 1);
    assert.equal(_spy.close, 0);
});

test('data-action button OUTSIDE #newBranchModal scope → no callback fires', () => {
    resetSpy();
    _capturedListener(makeClick({ action: 'closeNewBranchModal', inScope: false }));
    assert.equal(_spy.close, 0);
    assert.equal(_spy.create, 0);
});
...
test('second mountNewBranchModal call is a no-op', () => {
